 ...
 */
public class GICSSoapClientDemo
{
	...
	enum QCType
	{
		...
		not_checked,
		checked_no_faults,
		checked_minor_faults,
		checked_major_faults,
		invalidated
	}

	private final GICSService cm2Manager;
	...
	public GICSSoapClientDemo(String domain, String patientZeroSignerIDType, String patientZeroSignerID)
	{
		...
		try
		{
			...
			ConsentLightDTO patientZeroConsentLight = cm2Manager.getAllConsentsForSignerIds(domain, patientZeroSignerIDs, false).stream().findFirst().orElse(null);
			...
			patientZero = cm2Manager.getConsent(patientZeroConsentLight.getKey());
			patientZero.getKey().setSignerIds(patientZeroSignerIDs);
		}
		catch (UnknownDomainException | UnknownSignerIdTypeException e)
		{
	...
	private void addConsentsForNextPersons(int number, QCType qcType, boolean withSignatures, boolean withScan)
	{
		try
		{
			int start = cm2Manager.getAllIdsForSignerIdType(domain, patientZeroSignerIDType).size();
		...
		catch (UnknownDomainException | UnknownSignerIdTypeException | UnknownModuleException | UnknownConsentTemplateException | InvalidVersionException | MissingRequiredObjectException | InvalidFreeTextException | MandatoryFieldsException | DuplicateEntryException | InternalException | RequirementsNotFullfilledException | InvalidParameterException e)
		{
