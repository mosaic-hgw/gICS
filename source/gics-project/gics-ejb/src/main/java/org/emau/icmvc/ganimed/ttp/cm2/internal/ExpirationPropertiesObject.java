...
public class ExpirationPropertiesObject extends PropertiesObject implements Serializable
{
	...
	protected final Period validDuration;
	...
	public String toPropertiesString()
	{
		...
		if (validDuration != null)
		{
			result += ExpirationProperties.VALIDITY_PERIOD + "=" + validDuration + ";";
		}
		if (expirationDate != null)
		{
			result += ExpirationProperties.EXPIRATION_DATE + "=" + new SimpleDateFormat(DEFAULT_DATE_FORMAT).format(expirationDate) + ";";
			result += ExpirationProperties.EXPIRATION_DATE_FORMAT + "=" + DEFAULT_DATE_FORMAT + ";";
		}
		return result;
	}
	 ...
	 * <li>contains the new one's fixed expiration date and this one's expiration period (if finalised is true and both expiration dates refer to days in the future)</li>
	 * </ul>
	 *
	 * @param expiration
	...
	public ExpirationPropertiesObject createMergedExpirationProperties(ExpirationPropertiesDTO expiration, boolean finalised)
	{
		if (expiration == null)
		{
			expiration = new ExpirationPropertiesDTO(null, null);
		}
		...
		// for finalised templates only allow to update future expiration properties (both, the old and the new expiration must refer to a day in the future))
		else if (Dates.isFutureDay(expirationDate) && Dates.isFutureDay(expiration.getFixedExpirationDate()))
		{
		...
		// on finalised do not update at all if one of the fixed expiration dates does not refer to a future day
		return this;
	}
	...
	public Period getValidDuration()
	{
		return validDuration != null ? Period.of(validDuration.getYears(), validDuration.getMonths(), validDuration.getDays()) : null;
	}
	...
	public boolean equals(Object obj)
	{
		...
		if (validDuration == null)
		{
			return other.validDuration == null;
		}
		else
			return validDuration.equals(other.validDuration);
	}

	@Override
	public String toString()
	{
		return "consent template properties: " + ExpirationProperties.EXPIRATION_DATE
			   + " = "
			   + expirationDate
			   + "; "
			   + ExpirationProperties.VALIDITY_PERIOD
			   + " = "
			   + validDuration;
	}
}
