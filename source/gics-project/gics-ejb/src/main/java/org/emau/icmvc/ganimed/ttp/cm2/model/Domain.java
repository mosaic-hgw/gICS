...
{
	@Serial
	private static final long serialVersionUID = -8052508647239403929L;
	...
	private String configString; // XML (or legacy CSV)
	@Transient
	...
	@Transient
	private ExpirationPropertiesObject expirationPropertiesObject;
	...
	public Domain(DomainDTO dto) throws VersionConverterClassException
	{
		super();
		name = dto.getName();
		updateInUse(dto.getLabel(), dto.getComment(), dto.getExternProperties(), dto.getExpirationProperties(), dto.getLogo());
		updateLockableFields(dto);
	...
	/**
	 * This method is called by JPA and initializes all transient fields.
	 */
	@PostLoad
	private void init()
	{
		initConfig();
		initExpirationPropertiesObject();
	}

	private void initExpirationPropertiesObject()
	{
		try
		{
	...
	{
		config = new DomainConfig(name, configString);
		DomainConfig oldConfig = new DomainConfig(config);
		config.normalize(name);

		if (!oldConfig.equals(config) || !DomainConfig.isXmlConfig(configString))
		{
			configString = configAsXml(name, config);
		}
	}
	...
	{
		if (config == null) // most likely unnecessary
		{
			initConfig();
		}
		return config;
	...
	{
		if (expirationPropertiesObject == null) // most likely unnecessary
		{
			initExpirationPropertiesObject();
		}
	...
	public void updateInUse(String label, String comment, String externProperties, ExpirationPropertiesDTO expiration, String logo)
	{
		updateInUse(label, comment, externProperties, expiration, logo, null, null);
	}

	/**
	 * CAUTION: sets the lastCheckedDate not to null if the parameter is null
	 *
	 * @param label
	 * @param comment
	 ...
	 */
	public void updateInUse(String label, String comment, String externProperties, ExpirationPropertiesDTO expiration, String logo, DomainConfig config, Date lastCheckedDate)
	{
		setLabel(label);
		...
		if (config != null && getConfig().updateUnlockedParts(config))
		{
			configString = configAsXml(name, getConfig());
		}
		if (lastCheckedDate != null)
			lastCheckTimestamp = new Timestamp(lastCheckedDate.getTime());
	}
	...
	private void updateLockableFields(DomainDTO dto)
	{
		...
		configString = configAsXml(name, config);
		expirationPropertiesObject = new ExpirationPropertiesObject(dto.getExpirationProperties());
		expirationProperties = expirationPropertiesObject.toPropertiesString();
		signerIdTypes.clear();
	...
	private void updateExpiration(ExpirationPropertiesDTO expiration)
	{
		...
		expirationPropertiesObject = getExpirationPropertiesObject().createMergedExpirationProperties(expiration, finalised);
		expirationProperties = expirationPropertiesObject.toPropertiesString();
	}
	...
	public DomainDTO toDTO()
	{
		...
		return new DomainDTO(name, label, ctVersionConverter, moduleVersionConverter, policyVersionConverter, config,
				comment, externProperties, logo, sIdTypes, finalised, new Date(createTimestamp.getTime()), new Date(updateTimestamp.getTime()),
				getExpirationPropertiesObject().toDTO(), fhirID, lastCheckTimestamp);
	}
	...
	public String toLongString()
	{
		...
		sb.append(logo != null && !logo.isEmpty() ? "'has a logo and " : "'has no logo and ");
	...
	private static String configAsXml(String domainName, DomainConfig config)
	{
		if (config == null)
		{
			config = new DomainConfig();
			config.normalize(domainName);
		}
		try
		{
			return config.toXml();
	...
	public void setLastCheckTimestamp(Timestamp lastCheckTimestamp)
	{
