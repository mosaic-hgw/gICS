...
@Entity
@Table(name = "qc_problem")
// automatic history
@Customizer(QCProblemHistCustomizer.class)
@Cache(isolation = CacheIsolationType.PROTECTED)
@UuidGenerator(name = "FHIR_ID_QCP")
public class QCProblem implements Serializable, FhirDTOExporter<QCProblemDTO>
{
	@Serial
	private static final long serialVersionUID = 5751042339769883879L;

	@EmbeddedId
	private QCProblemKey key;
	...
	@GeneratedValue(generator = "FHIR_ID_QCP")
	...
	public QCProblem(QCProblemDTO dto, QC qc)
	{
		this.key = new QCProblemKey(qc.getKey(), dto.getTypeRef());
		this.qc = qc;
		update(dto, null);
	}
	...
	public void update(QCProblemDTO dto, Timestamp timestamp)
	{
		QCProblemDTO oldProblem = toDTO();
		// type and ref are part of PK and cannot be updated
		status = dto.getStatus().name();
		formValue = dto.getFormValue();
		scanValue = dto.getScanValue();
		commentExtern = dto.getCommentExtern();
		commentIntern = dto.getCommentIntern();

		timestamp = timestamp == null ? new Timestamp(System.currentTimeMillis()) : timestamp;
		if (createTimestamp == null)
		{
			createTimestamp = dto.getCreatedAt() == null ? timestamp : dto.getCreatedAt();
		}

		QCProblemDTO newProblem = toDTO();

		oldProblem.setUpdatedAt(null);
		newProblem.setUpdatedAt(null);
	...
	@Override
	public QCProblemDTO toDTO()
	{
		return new QCProblemDTO(getType(), getStatus(), getCreateTimestamp(), getUpdateTimestamp(),
	...
	public boolean equals(Object o)
	{
		...
		if (!(o instanceof QCProblem qcProblem))
			return false;

		return new EqualsBuilder()
				.append(getKey(), qcProblem.getKey()) // qc_key, type, and ref
				.append(getStatus(), qcProblem.getStatus())
				.append(getFormValue(), qcProblem.getFormValue())
				.append(getScanValue(), qcProblem.getScanValue())
				.append(getCommentExtern(), qcProblem.getCommentExtern())
				.append(getCommentIntern(), qcProblem.getCommentIntern())
				.append(getCreateTimestamp(), qcProblem.getCreateTimestamp())
				.append(getUpdateTimestamp(), qcProblem.getUpdateTimestamp())
				.isEquals();
	}
	...
	public int hashCode()
	{
		return new HashCodeBuilder(17, 37)
				.append(getKey()) // qc_key, type, and ref
				.append(getStatus())
				.append(getFormValue())
				.append(getScanValue())
	...
	public String toString()
	{
		return new ToStringBuilder(this, FlexibleToStringStyle.THS_TO_STRING_STYLE)
				.append("key", key) // qc_key, type, and ref
