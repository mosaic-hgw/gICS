...
{
	@EJB
	private NotificationManagerBean notificationManager;
	...
	public void finaliseAllForDomain(String domainName)
			throws InvalidParameterException, UnknownDomainException, InvalidVersionException, UnknownPolicyException,
			UnknownConsentTemplateException, UnknownModuleException, RequirementsNotFullfilledException, VersionConverterClassException
	{
		logger.info("finalise domain with name '{}' including all its templates, modules, and policies", domainName);
		checkParameter(domainName, "domainName");
		checkAllowedDomain(domainName);
		dad.finaliseAllForDomain(domainName);
		logger.info("domain '{}' including all its templates, modules, and policies finalised", domainName);
	}

	@Override
	public void finaliseDomain(String domainName)
			throws InvalidParameterException, UnknownDomainException, VersionConverterClassException
	{
		logger.info("finalise domain with name '{}'", domainName);
		checkParameter(domainName, "domainName");
		checkAllowedDomain(domainName);
		dad.finaliseDomain(domainName);
		logger.info("domain '{}' finalised", domainName);
	}

	@Override
	public void updateDomain(DomainDTO dto) throws InvalidParameterException, ObjectInUseException, UnknownDomainException, VersionConverterClassException
	{
		...
		logger.info("domain '{}' updated", dto.getName());
	}

	@Override
	public void updateDomainInUse(String domainName, String label, String logo, String externProperties, ExpirationPropertiesDTO expirationProperties, String comment, DomainConfig config)
			throws InvalidParameterException, UnknownDomainException, VersionConverterClassException
	{
		...
		if (config != null)
		{
			checkParameter(config, "domainConfig");
		}
		checkAllowedDomain(domainName);
		dad.updateDomainInUse(domainName, label, logo, externProperties, expirationProperties, comment, config);
			...
			RequirementsNotFullfilledException, UnknownDomainException, UnknownModuleException, UnknownConsentTemplateException, IllegalCompositionException
	{
		...
		logger.info("{} added", consentTemplateDTO);
	}
	...
	public void updateConsentTemplate(ConsentTemplateDTO dto, boolean finaliseRelatedEntities)
			...
			UnknownConsentTemplateException, UnknownDomainException, UnknownModuleException, IllegalCompositionException
	{
		...
		logger.info("{} updated", dto.getKey());
	}
	...
	public void addModule(ModuleDTO moduleDTO, boolean finaliseRelatedEntities)
			...
			UnknownDomainException, UnknownPolicyException, IllegalCompositionException
	{
	...
	public void updateModule(ModuleDTO dto, boolean finaliseRelatedEntities)
			throws InvalidParameterException, InvalidVersionException, ObjectInUseException,
			RequirementsNotFullfilledException, UnknownDomainException, UnknownModuleException, UnknownPolicyException, IllegalCompositionException
	{
		logger.info("update {}", dto.getKey());
	...
	{
		logger.info("add signer id type {} to domain {}", signerIdTypeName, domainName);
		checkParameter(domainName, "domainName");
		checkParameter(signerIdTypeName, "signerIdTypeName");
		checkAllowedDomain(domainName);
		dad.addSignerIdType(domainName, signerIdTypeName);
		logger.info("signer id type {} successfully added to domain {}", signerIdTypeName, domainName);
	}
	...
	{
		logger.info("remove signer id type {} from domain {}", signerIdTypeName, domainName);
		checkParameter(domainName, "domainName");
		checkParameter(signerIdTypeName, "signerIdTypeName");
		checkAllowedDomain(domainName);
		dad.deleteSignerIdType(domainName, signerIdTypeName);
		logger.info("signer id type {} removed from domain {}", signerIdTypeName, domainName);
	}
	...
	{
		logger.info("update signerIdType with name {} within domain {}", signerIdTypeName, domainName);
		checkParameter(domainName, "domainName");
		checkParameter(signerIdTypeName, "signerIdTypeName");
		checkAllowedDomain(domainName);
		dad.updateSignerIdType(domainName, signerIdTypeName, label, comment);
		logger.info("signerIdType {} within domain {} updated", signerIdTypeName, domainName);
	}

	@Override
	public void processPolicyValidityChanges(List<DomainDTO> domains)
			throws UnknownSignerIdTypeException, UnknownSignerIdException, InvalidVersionException, UnknownDomainException, VersionConverterClassException
	{
		notificationManager.processChangedPolicies(domains);
	}

	@Override
	public void processPoliciyValidityChangesForAllDomains() throws UnknownSignerIdTypeException, UnknownSignerIdException, InvalidVersionException, UnknownDomainException, VersionConverterClassException
	{
		notificationManager.processChangedPoliciesForAllDomains();
	}
}
