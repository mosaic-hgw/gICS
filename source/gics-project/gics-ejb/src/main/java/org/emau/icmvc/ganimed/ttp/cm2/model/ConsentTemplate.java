...
public class ConsentTemplate implements Serializable, FhirDTOExporter<ConsentTemplateDTO>
{
	...
	@Transient
	private ValidFromPropertiesObject validFromPropertiesObject = null;
	@Column(name = "VALID_FROM_PROPERTIES")
	private String validFromProperties;
			...
			RequirementsNotFullfilledException, UnknownModuleException, UnknownDomainException
	{
		...
		this.validFromPropertiesObject = new ValidFromPropertiesObject(dto.getValidFromProperties());
		this.validFromProperties = validFromPropertiesObject.toPropertiesString();
	...
	public void loadPropertiesFromString()
	{
		try
		{
			...
			validFromPropertiesObject = new ValidFromPropertiesObject(validFromProperties);
		}
			...
			throws InvalidFreeTextException, InvalidVersionException, UnknownDomainException, UnknownModuleException, InvalidParameterException
	{
		...
		updateValidFrom(dto.getValidFromProperties());
			...
			ObjectInUseException, RequirementsNotFullfilledException, UnknownDomainException, UnknownModuleException, InvalidVersionException, InvalidParameterException
	{
		...
		updateInUse(dto, false); // also fully updates expirationProperties and validFromProperties for non-finalised template
	...
	private void updateValidFrom(ValidFromPropertiesDTO validFrom)
	{
		// for finalized templates only allow to update future validFrom properties (existing as well as new date must be in the future)
		this.validFromPropertiesObject = validFromPropertiesObject.createMergedProperties(validFrom, finalised);
		this.validFromProperties = validFromPropertiesObject.toPropertiesString();
	}

	public ValidFromPropertiesObject getValidFromPropertiesObject()
	{
		if (validFromPropertiesObject == null)
		{
			loadPropertiesFromString();
		}
		return validFromPropertiesObject;
	}

	public void setValidFromPropertiesObject(ValidFromPropertiesObject validFromPropertiesObject)
	{
		this.validFromPropertiesObject = validFromPropertiesObject;
	}
	...
	public void setValidFromProperties(String validFromProperties)
	{
		this.validFromProperties = validFromProperties;
		if (validFromPropertiesObject == null)
		{
			loadPropertiesFromString();
		}
	...
	public ConsentTemplateDTO toDTO() throws InvalidVersionException, UnknownDomainException
	{
		...
		result.setExpirationProperties(expirationPropertiesObject != null ? expirationPropertiesObject.toDTO() : null);
		result.setValidFromProperties(validFromPropertiesObject != null ? validFromPropertiesObject.toDTO() : null);
		...
		final Set<ConsentTemplateKeyDTO> objectionMappings = new HashSet<>();
		final Set<ConsentTemplateKeyDTO> optOutConsentMappings = new HashSet<>();
		...
		for (MappedConsentTemplate m : getMappedConsentTemplates())
		{
			...
			switch (consentTemplateTo.getType())
			{
				...
				case OBJECTION -> objectionMappings.add(keyDTO);
				case CONSENT_OPT_OUT -> optOutConsentMappings.add(keyDTO);
			}
		}
		...
		result.setMappedObjectionTemplates(objectionMappings);
		result.setMappedOptOutConsentTemplates(optOutConsentMappings);
