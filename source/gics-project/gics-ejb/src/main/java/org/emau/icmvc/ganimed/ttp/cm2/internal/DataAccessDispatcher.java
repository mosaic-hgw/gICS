...
public class DataAccessDispatcher
{
	...
	static
	{
		...
		fhirIdObjects.put(QCProblemDTO.class.getName(), QCProblem.class);
		fhirIdObjects.put(QCProblemHistoryDTO.class.getName(), QCProblemHist.class);
			...
			throws UnknownSignerIdTypeException, InvalidVersionException, UnknownDomainException
	{
		...
		{
			LOGGER.fatal("Failed to get the current policy status for policy key {} and signer IDs {}", policyKey, signerIds, shouldNeverHappen);
		}
	...
	public void addDomain(DomainDTO domainDTO) throws DuplicateEntryException, VersionConverterClassException
	{
		...
		try
		{
			...
			{
				LOGGER.info("no label is set for new domain '{}' - set it to the name of the domain", domainDTO.getName());
	...
	public void finaliseAllForDomain(String domainName)
			throws UnknownDomainException, InvalidVersionException, UnknownPolicyException,
			UnknownConsentTemplateException, UnknownModuleException, RequirementsNotFullfilledException, VersionConverterClassException
	{
		ACCESS_RWL.writeLock().lock();
		try
		{
			OrgDatCache.updateDomain(dao.finaliseAllForDomain(domainName));
		}
		finally
		{
			ACCESS_RWL.writeLock().unlock();
	...
	public void updateDomain(DomainDTO domainDTO) throws ObjectInUseException, UnknownDomainException, VersionConverterClassException
	{
		LOGGER.debug("updateDomain {}", domainDTO);
		ACCESS_RWL.writeLock().lock();
		try
		{
	...
	public void updateDomainInUse(String domainName, String label, String logo, String externProperties, ExpirationPropertiesDTO expiration, String comment, DomainConfig config)
			throws UnknownDomainException, VersionConverterClassException
	{
		if (LOGGER.isDebugEnabled())
		{
		...
		try
		{
			if (expiration == null)
			{
				expiration = new ExpirationPropertiesDTO();
			}
			DomainDTO result = dao.updateDomainInUse(domainName, label, logo, externProperties, expiration, comment, config);
			OrgDatCache.updateDomain(result);
		}
		finally
		{
			...
			RequirementsNotFullfilledException, UnknownDomainException, UnknownModuleException, UnknownConsentTemplateException, IllegalCompositionException
	{
		...
		try
		{
			if (LOGGER.isDebugEnabled())
			{
				LOGGER.debug("addConsentTemplate {}", consentTemplateDTO);
			}
			...
			//>>> Only for OPT_OUT
			validateOptOutConstentTemplate(consentTemplateDTO);
			//<<<
			...
			{
				updateAssignedModulesInCache(consentTemplateDTO);
			}
		}
		finally
		{
	...
	private static void validateOptOutConstentTemplate(ConsentTemplateDTO consentTemplateDTO) throws RequirementsNotFullfilledException
	{
		if (ConsentTemplateType.CONSENT_OPT_OUT.equals(consentTemplateDTO.getType()))
		{
			for (AssignedModuleDTO assignedModule : consentTemplateDTO.getAssignedModules())
			{
				boolean hasNotMaxOneDisplayCheckbox = assignedModule.getDisplayCheckboxes().size() > 1;
				boolean wrongStatus = !assignedModule.getDisplayCheckboxes().isEmpty() && !ConsentStatus.ACCEPTED_OPT_OUT.equals(assignedModule.getDisplayCheckboxes().getFirst());
				boolean wrongDefaultStatus = !ConsentStatus.ACCEPTED_OPT_OUT.equals(assignedModule.getDefaultConsentStatus());
				if (hasNotMaxOneDisplayCheckbox || wrongStatus || wrongDefaultStatus)
				{
					throw new RequirementsNotFullfilledException(
							"ConsentTemplates of type " + ConsentTemplateType.CONSENT_OPT_OUT.name() + " must have a maximum of one display checkbox with status "
							+ ConsentStatus.ACCEPTED_OPT_OUT.name() + " as default");
				}
			}
		}
	}
			...
			throws InvalidVersionException, RequirementsNotFullfilledException, UnknownConsentTemplateException, UnknownDomainException
	{
		...
		try
		{
			...
			{
				updateAssignedModulesInCache(ctDTO);
			}
		}
		finally
		{
	...
	public void updateConsentTemplate(ConsentTemplateDTO consentTemplateDTO, boolean finaliseRelatedEntities) throws DuplicateEntryException,
	...
	{
		LOGGER.debug("update {}", consentTemplateDTO);
		ACCESS_RWL.writeLock().lock();
		try
		{
			//>>> Only for OPT_OUT
			validateOptOutConstentTemplate(consentTemplateDTO);
			//<<<
			consentTemplateDTO = dao.updateConsentTemplate(consentTemplateDTO, finaliseRelatedEntities);
			OrgDatCache.updateConsentTemplate(consentTemplateDTO);
			if (finaliseRelatedEntities && consentTemplateDTO.getFinalised())
			{
				updateAssignedModulesInCache(consentTemplateDTO);
			}
	...
	{
		LOGGER.debug("update {} (updateInUse - only non-critical fields are updated)", dto);
		ACCESS_RWL.writeLock().lock();
		try
		{
			...
			RequirementsNotFullfilledException, UnknownDomainException, UnknownPolicyException, IllegalCompositionException
	{
		...
		try
		{
			...
			{
				updateAssignedPoliciesInCache(moduleDTO);
			}
		}
		finally
		{
	...
	public void updateModule(ModuleDTO moduleDTO, boolean finaliseRelatedEntities) throws InvalidParameterException, InvalidVersionException,
	...
	private void updateAssignedModulesInCache(ConsentTemplateDTO ctDTO)
	{
		for (AssignedModuleDTO assignedModuleDTO : ctDTO.getAssignedModules())
		{
			...
			updateAssignedPoliciesInCache(assignedModuleDTO.getModule());
		}
	}

	private void updateAssignedPoliciesInCache(ModuleDTO moduleDTO)
	{
	...
	public ConsentDTO addConsentOptOut(ConsentTemplateKeyDTO templateKeyDTO, Set<SignerIdDTO> signerIds)
	...
	{
		return addConsentOptOut(templateKeyDTO, signerIds, null);
	}

	public ConsentDTO addConsentOptOut(ConsentTemplateKeyDTO templateKeyDTO, Set<SignerIdDTO> signerIds, String notificationClientID)
			throws UnknownSignerIdTypeException, InvalidParameterException, UnknownConsentTemplateException, InvalidVersionException, RequirementsNotFullfilledException, UnknownModuleException,
			InvalidFreeTextException, UnknownDomainException, MissingRequiredObjectException, MandatoryFieldsException, DuplicateEntryException
	{
		ACCESS_RWL.writeLock().lock();
		try
		{
			ConsentKeyDTO consentKeyDTO = new ConsentKeyDTO(templateKeyDTO, signerIds, new Date());
			boolean sendNoti = StringUtils.isNotBlank(notificationClientID);
			Map<PolicyKeyDTO, Boolean> previousPolicyStates = sendNoti ? getPolicyStates(consentKeyDTO, null) : null;

			ConsentDTO consentDTO = dao.addConsentOptOut(consentKeyDTO);

			Map<PolicyKeyDTO, Boolean> currentPolicyStates = sendNoti ? getPolicyStates(consentKeyDTO, null) : null;
			//TODO PP: Check VALID_FROM_PROPS before send notification
			if (sendNoti)
	...
	public long countSignedPolicies(String domainName) throws UnknownDomainException
	{
		...
		try
		{
			return dao.countSignedPolicies(domainName);
		}
		finally
		{
			ACCESS_RWL.readLock().unlock();
			...
			UnknownModuleException, UnknownConsentTemplateException, UnknownDomainException, InvalidParameterException
	{
		...
		try
		{
			dao.validateConsentReturningObjects(consentDTO, allowRevoke, validateScan);
		}
		finally
		{
			ACCESS_RWL.readLock().unlock();
			...
			throws InvalidParameterException, InvalidVersionException, UnknownDomainException, UnknownConsentTemplateException, UnknownSignerIdTypeException, UnknownConsentException
	{
		...
		{
			Map<PolicyKeyDTO, Boolean> previousPolicyStates = StringUtils.isNotBlank(notificationClientID) ? getPolicyStates(consentKeyDTO, null) : null;
			QC qc = dao.setQcForConsent(consentKeyDTO, qcDTO, notificationClientID);
			...
			notificationSender.sendNotification(notificationClientID, consentKeyDTO, previousPolicyStates, getPolicyStates(consentKeyDTO, null), "SetQcForConsent", null,
					Collections.singletonMap("qc", qc.toDTO()));
			// }
		}
		finally
		{
			...
			throws UnknownSignerIdTypeException, UnknownConsentTemplateException, InvalidVersionException, UnknownDomainException, UnknownConsentException
	{
		...
		try
		{
			return dao.getQCProblemHistoryForQCProblem(qcProblemDTO, consentKeyDTO);
		}
		finally
		{
			ACCESS_RWL.readLock().unlock();
	...
	public List<StatisticDTO> getStatsFromTo(Date from, Date to)
	{
		...
		try
		{
			return dao.getStatsFromTo(from, to);
		}
		finally
		{
			ACCESS_RWL.readLock().unlock();
			...
			throws UnknownSignerIdTypeException, UnknownConsentTemplateException, InvalidVersionException, UnknownDomainException, UnknownConsentException
	{
		...
		try
		{
			return dao.getVirtualPersonForConsentKey(consentKeyDTO);
		}
		finally
		{
			ACCESS_RWL.readLock().unlock();
