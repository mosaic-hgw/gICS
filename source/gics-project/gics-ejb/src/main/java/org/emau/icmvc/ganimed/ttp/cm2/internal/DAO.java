...
import org.emau.icmvc.ganimed.ttp.cm2.model.QCKey;
import org.emau.icmvc.ganimed.ttp.cm2.model.QCKey_;
import org.emau.icmvc.ganimed.ttp.cm2.model.QCProblem;
import org.emau.icmvc.ganimed.ttp.cm2.model.QCProblemHist;
import org.emau.icmvc.ganimed.ttp.cm2.model.QCProblemHistKey_;
import org.emau.icmvc.ganimed.ttp.cm2.model.QCProblemHist_;
...
public class DAO
{
	...
	private Map<ModuleKeyDTO, Module> mapModules(ConsentTemplateDTO consentTemplateDTO, Domain domain)
			throws InvalidVersionException, UnknownDomainException, UnknownModuleException, IllegalCompositionException
	{
		...
		for (AssignedModuleDTO assignedModuleDTO : consentTemplateDTO.getAssignedModules())
		{
			...
			if (moduleNames.contains(moduleName))
			{
				...
				LOGGER.warn(message);
				throw new IllegalCompositionException(message, moduleName, IllegalCompositionException.ItemType.MODULE);
			}
			...
			for (ModulePolicy mp : module.getModulePolicies())
			{
				...
				if (policyNames.contains(policyName))
				{
					...
					LOGGER.warn(message);
					throw new IllegalCompositionException(message, policyName, IllegalCompositionException.ItemType.POLICY);
				}
	...
	public VirtualPerson createVirtualPerson(Domain domain, Set<SignerIdDTO> signerIdDTOs) throws UnknownSignerIdTypeException
	{
	...
	private Map<SignedPolicy, Long> getSignedPoliciesWithExpirationDates(List<SignedPolicy> signedPolicies)
	{
		...
		try
		{
			Consent consent = signedPolicies.getFirst().getConsent();
	...
	public static ConsentStatusType checkPolicies(CheckConsentConfig config, List<CachedSignedPolicy> signedPolicies, Domain domain)
	{
		try
		{
			CachedSignedPolicy currentSignedPolicy = getCurrentPolicy(config, signedPolicies, domain);
			return currentSignedPolicy.getConsentStatus().getConsentStatusType();
		}
		catch (ObjectNotFoundException e)
		{
			//can be ignored for checking the status
	 ...
	 * Or version is equal and {@link #consentDateIsNewer(CachedSignedPolicy, CachedSignedPolicy, CheckConsentConfig)}
	 *
	 * @param signedPolicy
	 ...
	 */
	private static boolean versionIsHigher(CachedSignedPolicy signedPolicy, CachedSignedPolicy currentSignedPolicy, CheckConsentConfig config)
	{
		return signedPolicy.getVersion() > currentSignedPolicy.getVersion()
			   || signedPolicy.getVersion() == currentSignedPolicy.getVersion() && consentDateIsNewer(signedPolicy, currentSignedPolicy, config);
	}

	/**
	 * Checks if the legalConsentDate of signedPolicy is newer than the legalConsentDate of currentSignedPolicy
	 * Or legalConsentDate is equal or on same day and signedPolicy consentStatus is kind of DECLINED
	 *
	 * @param signedPolicy
	 * 		Possible new signed policy
	 * @param currentSignedPolicy
	 ...
	 * @return true if signedPolicy should replace the currentSignedPolicy
	 */
	private static boolean consentDateIsNewer(CachedSignedPolicy signedPolicy, CachedSignedPolicy currentSignedPolicy, CheckConsentConfig config)
	{
		// the check for same day is really fast, 100 calculations take ~0,1ms. So additionally putting the legalConsentDate (as start of day)
		// in the CachedSignedPolicy might take more memory than it improves performance
		if (DateUtils.isSameDay(new Date(signedPolicy.getLegalConsentDate()),
				new Date(currentSignedPolicy.getLegalConsentDate())))
		{
			if (isDeclinedStatus(signedPolicy.getConsentStatus().getConsentStatusType(), config))
				return true;
			else if (isDeclinedStatus(currentSignedPolicy.getConsentStatus().getConsentStatusType(), config))
				return false;
		}
	...
	/**
	 * checks if the given status is considered as declined with regard to the CheckConsentConfig
	 */
	private static boolean isDeclinedStatus(ConsentStatusType type, CheckConsentConfig config)
	{
		return ConsentStatusType.DECLINED.equals(type) || (config.getUnknownStateIsConsideredAsDecline() && ConsentStatusType.UNKNOWN.equals(type));
	}
	...
	private static CachedSignedPolicy getCurrentPolicy(CheckConsentConfig config, List<CachedSignedPolicy> signedPolicies, Domain domain) throws ObjectNotFoundException
	{
		...
		for (CachedSignedPolicy signedPolicy : signedPolicies)
		{
			...
			if (domain.getConfig().getPoliciesConfig().isPermanentRevoke() && isDeclinedStatus(status, config))
			{
				currentSignedPolicy = signedPolicy;
				break;
			}
			// only ignore if unknown is not considered as declined
			if (ConsentStatusType.UNKNOWN.equals(status) && !config.getUnknownStateIsConsideredAsDecline())
			{
				continue;
			}
			boolean takeHighestVersion = domain.getConfig().getPoliciesConfig().isTakeHighestVersionInsteadOfNewest();
			// Update currentSignedPolicy
			if (currentSignedPolicy == null
				|| takeHighestVersion && versionIsHigher(signedPolicy, currentSignedPolicy, config)
				|| !takeHighestVersion && consentDateIsNewer(signedPolicy, currentSignedPolicy, config))
			{
				currentSignedPolicy = signedPolicy;
			}
		}
			...
			for (Entry<ConsentField, String> entry : config.getFilter().entrySet())
			{
				...
				switch (entry.getKey())
				{
					...
					case QC_TYPE:
						...
						{
							QualityControlConfig qc = domain.getConfig().getQualityControlConfig();
							if (!qc.hasType(qcType, QCTypeStatus.VALID) && !qc.hasType(qcType, QCTypeStatus.INVALID))
							{
								String message = "unknown qc status '" + qcType + "' for filter-clause within generateWhereForConsent(): "
												 + "neither in valid " + qc.getValidQcTypeValues() + " nor in invalid " + qc.getInvalidQcTypeValues() + " qc types";
	...
	private Predicate getWhereForDate(CriteriaBuilder criteriaBuilder, Root<Consent> root, PaginationConfig config)
	{
	...
	private Predicate getWhereForType(CriteriaBuilder criteriaBuilder, Root<Consent> root, String domainName, PaginationConfig config) throws ObjectNotFoundException
	{
		if (config.getTemplateType() != null)
		{
			...
			try
			{
				VersionConverter vc = VersionConverterCache.getCTVersionConverter(domainName);
				...
				{
					Path<String> nameCriteria = root.get(Consent_.key).get(ConsentKey_.ctKey).get(ConsentTemplateKey_.name);
					Path<Integer> versionCriteria = root.get(Consent_.key).get(ConsentKey_.ctKey).get(ConsentTemplateKey_.version);
					for (ConsentTemplateDTO templateDTO : templateDTOs)
					{
						Predicate nameCond = criteriaBuilder.equal(nameCriteria, templateDTO.getKey().getName());
						Predicate versionCond = criteriaBuilder.equal(versionCriteria, vc.stringToInt(templateDTO.getKey().getVersion()));
						result = criteriaBuilder.or(result, criteriaBuilder.and(nameCond, versionCond));
					}
				}
			}
			catch (UnknownDomainException | InvalidVersionException impossible)
			{
	...
	private Path<?> generateSortExpressionForConsent(ConsentField sortField, Root<Consent> root)
	{
		Path<?> order = null;
		if (sortField != null)
		{
			...
			throws InvalidVersionException, UnknownSignerIdTypeException, UnknownConsentTemplateException, UnknownDomainException, InternalException, InvalidParameterException
	{
		...
		try
		{
			...
			Map<ModuleKeyDTO, Module> modules = validateConsentReturningObjects(consentDTO, true, true);
			Consent newConsent = new Consent(consent.getConsentTemplate(), consentDTO, modules, usableVP, domain.getConfig());
			em.persist(newConsent);
			// den alten nicht loeschen! bisherige anfragen muessen valid bleiben

			ConsentCache.addConsent(usableVP.getId(), convertToSignerIdKeys(domain, Set.of(signerIdDTO)), getSignedPoliciesWithExpirationDates(consent.getSignedPolicies()),
					consent);
		}
	...
	private List<Consent> getConsentsForPerson(Domain domain, VirtualPerson person)
	{
		...
		Predicate predicate = criteriaBuilder.and(criteriaBuilder.equal(root.get(Consent_.key).get(ConsentKey_.virtualPersonId), person.getId()),
				criteriaBuilder.equal(root.get(Consent_.key).get(ConsentKey_.ctKey).get(ConsentTemplateKey_.domainName), domain.getName()));
		criteriaQuery.select(root).where(predicate);
		return new ArrayList<>(em.createQuery(criteriaQuery).getResultList());
	}
	...
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	public DomainDTO finaliseAllForDomain(String domainName)
			throws UnknownDomainException, InvalidVersionException, UnknownPolicyException, UnknownModuleException,
			RequirementsNotFullfilledException, UnknownConsentTemplateException
	{
		EM_RWL.writeLock().lock();
		try
		{
			for (PolicyDTO p : OrgDatCache.listPoliciesCFEU(domainName, false)
					.stream().filter(p -> !p.getFinalised()).toList())
			{
				OrgDatCache.updatePolicy(finalisePolicyInternal(p.getKey()));
			}

			for (ModuleDTO m : OrgDatCache.listModulesCFEU(domainName, false)
					.stream().filter(m -> !m.getFinalised()).toList())
			{
				OrgDatCache.updateModule(finaliseModuleInternal(m.getKey(), false));
			}

			for (ConsentTemplateDTO t : OrgDatCache.listConsentTemplatesCFEU(domainName, false)
					.stream().filter(t -> !t.getFinalised()).toList())
			{
				OrgDatCache.updateConsentTemplate(finaliseTemplateInternal(t.getKey(), false));
			}

			return finaliseDomainInternal(domainName);
		}
		finally
		{
			EM_RWL.writeLock().unlock();
	...
	private DomainDTO finaliseDomainInternal(String domainName) throws UnknownDomainException
	{
	...
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	public DomainDTO updateDomainInUse(String domainName, String label, String logo, String externProperties, ExpirationPropertiesDTO expiration, String comment, DomainConfig config)
			throws UnknownDomainException
	{
		...
		{
			Domain domain = getDomain(domainName);
			domain.updateInUse(label, comment, externProperties, expiration, logo, config, null);
			return domain.toDTO();
		}
		finally
		{
	...
	public ConsentTemplateDTO addConsentTemplate(ConsentTemplateDTO consentTemplateDTO, boolean finaliseRelatedEntities)
			...
			RequirementsNotFullfilledException, UnknownDomainException, UnknownModuleException, UnknownConsentTemplateException, IllegalCompositionException
	{
		EM_RWL.writeLock().lock();
		try
		{
			...
			throws InvalidVersionException, RequirementsNotFullfilledException, UnknownConsentTemplateException, UnknownDomainException
	{
		...
		try
		{
			return finaliseTemplateInternal(keyDTO, finaliseRelatedEntities);
		}
		finally
		{
			EM_RWL.writeLock().unlock();
	...
	private ConsentTemplateDTO finaliseTemplateInternal(ConsentTemplateKeyDTO keyDTO, boolean finaliseRelatedEntities)
			throws UnknownConsentTemplateException, InvalidVersionException, UnknownDomainException, RequirementsNotFullfilledException
	{
		ConsentTemplate consentTemplate = getConsentTemplate(createConsentTemplateKey(keyDTO));
		try
		{
	...
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	public ConsentTemplateDTO updateConsentTemplate(ConsentTemplateDTO dto, boolean finaliseRelatedEntities) throws
			...
			RequirementsNotFullfilledException, UnknownConsentTemplateException, UnknownDomainException, UnknownModuleException, InvalidParameterException, IllegalCompositionException
	{
		EM_RWL.writeLock().lock();
		try
		{
	...
	public ModuleDTO addModule(ModuleDTO moduleDTO, boolean finaliseRelatedEntities) throws DuplicateEntryException, InvalidVersionException,
			RequirementsNotFullfilledException, UnknownDomainException, UnknownPolicyException, IllegalCompositionException
	{
		EM_RWL.writeLock().lock();
		try
		{
			...
			throws InvalidVersionException, RequirementsNotFullfilledException, UnknownDomainException, UnknownModuleException
	{
		...
		try
		{
			return finaliseModuleInternal(keyDTO, finaliseRelatedEntities);
		}
		finally
		{
			EM_RWL.writeLock().unlock();
	...
	private ModuleDTO finaliseModuleInternal(ModuleKeyDTO keyDTO, boolean finaliseRelatedEntities)
			throws UnknownModuleException, InvalidVersionException, UnknownDomainException, RequirementsNotFullfilledException
	{
		Module module = getModule(new ModuleKey(VersionConverterCache.getModuleVersionConverter(keyDTO.getDomainName()), keyDTO));
		try
		{
	...
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	public ModuleDTO updateModule(ModuleDTO dto, boolean finaliseRelatedEntities) throws InvalidParameterException, InvalidVersionException,
			ObjectInUseException, RequirementsNotFullfilledException, UnknownDomainException, UnknownModuleException, UnknownPolicyException, IllegalCompositionException
	{
		EM_RWL.writeLock().lock();
		try
		{
	...
	public PolicyDTO finalisePolicy(PolicyKeyDTO keyDTO) throws InvalidVersionException, UnknownDomainException, UnknownPolicyException
	{
		...
		try
		{
			return finalisePolicyInternal(keyDTO);
		}
		finally
		{
			EM_RWL.writeLock().unlock();
	...
	private PolicyDTO finalisePolicyInternal(PolicyKeyDTO keyDTO) throws UnknownPolicyException, InvalidVersionException, UnknownDomainException
	{
		Policy policy = getPolicy(new PolicyKey(VersionConverterCache.getPolicyVersionConverter(keyDTO.getDomainName()), keyDTO));
		policy.finalise();
		return policy.toDTO();
			...
			InvalidFreeTextException, UnknownSignerIdTypeException, InvalidParameterException, RequirementsNotFullfilledException
	{
		...
		try
		{
			...
			if (consentDTO.getQualityControl() != null)
			{
				// validate qc problems (no duplicate type-refs and consistent with qc config)
				consentDTO.getQualityControl().validate(domain.getConfig().getQualityControlConfig(), "consent.qc");
			}

			Consent newConsent = new Consent(consentTemplate, consentDTO, modules, virtualPerson, domain.getConfig());
			newConsent.calculateAndSetSignedPolicyExpirationDates();
			em.persist(newConsent);
	...
	public long countSignedPolicies(String domainName) throws UnknownDomainException
	{
		...
		try
		{
			Domain domain = getDomain(domainName); // validate domain name
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<Long> cq = cb.createQuery(Long.class);
			Root<SignedPolicy> root = cq.from(SignedPolicy.class);
			Predicate predicate = cb.equal(root
					.get(SignedPolicy_.key)
					.get(SignedPolicyKey_.consentKey)
					.get(ConsentKey_.ctKey)
					.get(ConsentTemplateKey_.domainName), domain.getName());
	...
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	public Map<ModuleKeyDTO, Module> validateConsentReturningObjects(ConsentDTO consentDTO, boolean allowRevoke, boolean validateScan)
			throws UnknownDomainException, UnknownConsentTemplateException, InvalidVersionException,
			MissingRequiredObjectException, MandatoryFieldsException, UnknownModuleException, InvalidParameterException
	{
		...
		try
		{
			...
			//check if the type of the dto fits to the type of the templateKey
			if (consentDTO.getTemplateType() != null && !consentDTO.getTemplateType().equals(consentTemplate.getType()))
			{
				String message = "V8 - the given templateType " + consentDTO.getTemplateType() + " does not fit to the templateKeyType " + consentTemplate.getType();
				LOGGER.warn(message);
				throw new InvalidParameterException(message);
			}
			...
			boolean someMandatoryFieldsAccepted = !allowRevoke;
			boolean someMandatoryFieldsDeclined = false;
			boolean someOptionalFieldsAccepted = false;
			for (ModuleConsentTemplate moduleConsentTemplate : moduleConsentTemplates)
			{
				...
				if (moduleState == null)
				{
					String message = "V7 - missing module: " + moduleKeyDTO;
					LOGGER.warn(message);
				...
				if (!moduleConsentTemplate.getModule().getModulePolicies().isEmpty())
				{
					...
					if (moduleConsentTemplate.getMandatory())
					{
						...
						{
							someMandatoryFieldsAccepted = true;
						}
						else if (csType.equals(ConsentStatusType.DECLINED))
						{
							someMandatoryFieldsDeclined = true;
						}
						else
						{
					...
					{
						someOptionalFieldsAccepted = true;
					}
			...
			if (someMandatoryFieldsDeclined && someMandatoryFieldsAccepted)
			{
			...
			else if (someMandatoryFieldsDeclined && someOptionalFieldsAccepted)
			{
			...
			if ((someOptionalFieldsAccepted || someMandatoryFieldsAccepted) && !ConsentTemplateType.CONSENT_OPT_OUT.equals(consentDTO.getTemplateType()))
			{
				boolean scanMissing = validateScan && !consentDTO.hasScans();
				boolean scanMandatory = domain.getConfig().getScansConfig().isMandatory();
				boolean signatureMissing = !consentDTO.hasPatientSignature() || !consentDTO.hasPhysicianSignature();
				if (scanMissing && scanMandatory)
				{
					String message = "V1 - the scan of the consent is mandatory (as configured) if at least one module is consented. "
									 + consentDTO.getKey();
					LOGGER.warn(message);
					throw new MissingRequiredObjectException(message);
				}
				else if (scanMissing && signatureMissing)
				{
					String message = "V2 - need scan or signatures of both the patient and a physician if at least one module is consented. "
									 + consentDTO.getKey();
					LOGGER.warn(message);
					throw new MissingRequiredObjectException(message);
				}
				...
				{
					String message = "V3 - need signature dates if at least one module is consented."
									 + consentDTO.getKey();
					LOGGER.warn(message);
					throw new MissingRequiredObjectException(message);
				}
				...
				{
					String message = "V4 - consent date need to be after " + MINDATE_FOR_SIGNATURE + " if at least one module is consented. "
									 + consentDTO.getKey();
					LOGGER.warn(message);
					throw new MissingRequiredObjectException(message);
				}
			...
			{
				String message = "V5 - a given signature date need to be after " + MINDATE_FOR_SIGNATURE + ". "
								 + consentDTO.getKey();
				LOGGER.warn(message);
				throw new InvalidParameterException(message);
			}
			...
			for (ModuleKeyDTO moduleKeyDTO : consentDTO.getModuleStates().keySet())
			{
				...
				{
					String message = "V6 - the given " + moduleKeyDTO + " is not part of the given " + consentKeyDTO.getConsentTemplateKey();
					LOGGER.warn(message);
					throw new UnknownModuleException(message);
				}
				ModuleKey moduleKey = new ModuleKey(VersionConverterCache.getModuleVersionConverter(domain.getName()), moduleKeyDTO);
			...
			throws UnknownDomainException, UnknownConsentTemplateException, InvalidVersionException, InvalidParameterException, UnknownSignerIdTypeException, InternalException
	{
		...
		try
		{
			...
			try
			{
				Consent newConsent = new Consent(consentTemplate, tempConsentDTO, modules, virtualPerson, domain.getConfig());
				em.persist(newConsent);
	...
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	public QC setQcForConsent(ConsentKeyDTO consentKeyDTO, QCDTO qcDTO, String notificationClientID)
			throws InvalidParameterException, InvalidVersionException, UnknownDomainException, UnknownConsentTemplateException, UnknownSignerIdTypeException, UnknownConsentException
	{
		...
		try
		{
			...
			// validate qc problems (no duplicate type-refs and consistent with qc config)
			qcDTO.validate(domainObject.getConfig().getQualityControlConfig(), "qc");

			// save the old problems before updating
			HashSet<QCProblem> oldProblems = new HashSet<>(consent.getQc().getProblems());

			// update
			consent.updateQC(qcDTO, domainObject.getConfig().getQualityControlConfig());

			// remove old orphaned problems
			Set<QCProblemDTO.TypeRef> currentProblemTypeRefs = consent.getQc().getProblems().stream()
					.map(QCProblem::getTypeRef).collect(Collectors.toSet()); // the type-refs of the current problems
			oldProblems.removeIf(p -> currentProblemTypeRefs.contains(p.getTypeRef())); // leaves orphans only in old problem set
			oldProblems.forEach(p -> em.remove(p)); // removes old orphans from db
			...
			throws UnknownDomainException, UnknownConsentTemplateException, InvalidVersionException, UnknownSignerIdTypeException, UnknownConsentException
	{
		...
		{
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<QCHist> query = cb.createQuery(QCHist.class);
			Root<QCHist> root = query.from(QCHist.class);
			Predicate predicateKey = cb.equal(root.get(QCHist_.key).get(QCHistKey_.qcKey).get(QCKey_.consentKey), toConsentKey(consentKeyDTO));
			query.select(root).where(predicateKey);
			return em.createQuery(query).getResultList().stream().map(QCHist::toDTO).collect(Collectors.toList());
		}
		catch (ObjectNotFoundException e)
		{
	...
	public List<QCProblemHistoryDTO> getQCProblemHistoryForQCProblem(QCProblemDTO qcProblemDTO, ConsentKeyDTO consentKeyDTO)
			throws UnknownDomainException, InvalidVersionException, UnknownConsentTemplateException, UnknownSignerIdTypeException, UnknownConsentException
	{
		EM_RWL.readLock().lock();
		try
		{
			CriteriaBuilder cb = em.getCriteriaBuilder();
			CriteriaQuery<QCProblemHist> query = cb.createQuery(QCProblemHist.class);
			Root<QCProblemHist> root = query.from(QCProblemHist.class);

			Predicate predicateKey = cb.equal(root.get(QCProblemHist_.key).get(QCProblemHistKey_.qcKey), new QCKey(toConsentKey(consentKeyDTO)));
			predicateKey = cb.and(predicateKey,
					cb.equal(root.get(QCProblemHist_.key).get(QCProblemHistKey_.type), qcProblemDTO.getTypeRef().type()));
			predicateKey = cb.and(predicateKey,
					cb.equal(root.get(QCProblemHist_.key).get(QCProblemHistKey_.ref), qcProblemDTO.getTypeRef().ref()));

			query.select(root).where(predicateKey);

			return em.createQuery(query).getResultList().stream().map(QCProblemHist::toDTO).collect(Collectors.toList());
		}
		catch (ObjectNotFoundException e)
		{
			String message = consentKeyDTO + " not found";
	...
	private ConsentKey toConsentKey(ConsentKeyDTO consentKeyDTO)
			throws UnknownDomainException, UnknownConsentTemplateException, InvalidVersionException, ObjectNotFoundException, UnknownSignerIdTypeException
	{
		Domain domain = getDomain(consentKeyDTO.getConsentTemplateKey().getDomainName());
		ConsentTemplate ct = getConsentTemplate(
				new ConsentTemplateKey(VersionConverterCache.getCTVersionConverter(domain.getName()), consentKeyDTO.getConsentTemplateKey()));
		VirtualPerson vp = findVirtualPersonWithExactTheSignerIds(domain, convertToSignerIdKeys(domain, consentKeyDTO.getSignerIds()));
		return new ConsentKey(ct.getKey(), consentKeyDTO.getConsentDate(), vp.getId());
	}
			...
			throws UnknownDomainException, UnknownSignerIdException, UnknownSignerIdTypeException
	{
		...
		try
		{
			...
			Set<ConsentTemplateKeyDTO> ctKeyDTOs = getAllConsentsForSignerIds(domainName, Set.of(signerId), useAliases).stream()
					.filter(c -> c.getQualityControl().isQcPassed())
	...
	public StatisticDTO getFirstStats()
	{
		...
		try
		{
			...
			criteriaQuery.select(root).orderBy(criteriaBuilder.asc(root.get(Statistic_.stat_entry_id)));
	...
	public List<StatisticDTO> getStatsFromTo(Date from, Date to)
	{
		...
		try
		{
			...
			Root<Statistic> root = criteriaQuery.from(Statistic.class);
			criteriaQuery.select(root)
					.where(criteriaBuilder.between(root.get(Statistic_.entrydate), from, to));
			List<Statistic> queryResult = em.createQuery(criteriaQuery).getResultList();
	...
	public StatisticDTO updateStats() throws StatisticException
	{
		...
		try
		{
			...
			for (DomainDTO domainDTO : domains)
			{
				...
				//count all templateTypes
				for (ConsentTemplateType templateType : ConsentTemplateType.values())
				{
					paginationConfig.setTemplateType(templateType);
					long count = countConsentsForDomainWithFilter(domainDTO.getName(), paginationConfig);
					String statsKey = StatisticKeys.templateTypeKeys.get(templateType);
					stat.put(statsKey + StatisticKeys.PER_DOMAIN + domainDTO.getName(), count);
					stat.put(statsKey, stat.getOrDefault(statsKey, 0L) + count);
				}
				...
				// Signed policies
				stat.put(StatisticKeys.SIGNED_POLICIES + StatisticKeys.PER_DOMAIN + domainDTO.getName(), countSignedPolicies(domainDTO.getDomainName()));
				...
				for (String signerIdType : domainDTO.getSignerIdTypes())
				{
					...
					if (domainDTO.getConfig().getStatisticConfig().isCalculatePolicyDetails())
					{
				...
				if (domainDTO.getConfig().getStatisticConfig().isCalculateDocumentDetails())
				{
			...
			// All documents that are fully expired
			stat.put(StatisticKeys.DOCUMENTS_EXPIRED_FULL,
					domains.stream().mapToLong(d -> stat.getOrDefault(new StatisticKeys(StatisticKeys.DOCUMENTS_EXPIRED_FULL).perDomain(d.getName()).build(), 0L)).sum());

			// All signed policies
			stat.put(StatisticKeys.SIGNED_POLICIES,
					domains.stream().mapToLong(d -> stat.getOrDefault(new StatisticKeys(StatisticKeys.SIGNED_POLICIES).perDomain(d.getName()).build(), 0L)).sum());
		}
	...
	public ConsentDTO addConsentOptOut(ConsentKeyDTO consentKeyDTO)
			throws UnknownDomainException, InvalidVersionException, UnknownConsentTemplateException, RequirementsNotFullfilledException, UnknownSignerIdTypeException, InvalidParameterException,
			UnknownModuleException, InvalidFreeTextException, MissingRequiredObjectException, MandatoryFieldsException, DuplicateEntryException
	{
		EM_RWL.readLock().lock();
		//needed cause addConsent-call also uses the lock, but in case of a failure before the call, the lock needs to be released here too because unlocking a released lock throws an exception
		boolean unlocked = false;
		try
		{
			String domainName = consentKeyDTO.getDomainName();
			Domain domain = getDomain(domainName);
			ConsentTemplateKey templateKey = new ConsentTemplateKey(VersionConverterCache.getCTVersionConverter(domain.getName()), consentKeyDTO.getConsentTemplateKey());
			ConsentTemplate consentTemplate = getConsentTemplate(templateKey);
			if (!ConsentTemplateType.CONSENT_OPT_OUT.equals(consentTemplate.getType()))
			{
				throw new RequirementsNotFullfilledException("Consent template is not of type CONSENT_OPT_OUT. Use addConsent instead.");
			}

			ConsentTemplateDTO consentTemplateDTO = consentTemplate.toDTO();

			//convert the template to a filled consentDTO with the default-status set for each module and then add this consent
			ConsentDTO consentDTO = new ConsentDTO();
			consentDTO.setKey(consentKeyDTO);
			consentDTO.setTemplateType(consentTemplate.getType());
			for (AssignedModuleDTO assignedModuleDTO : consentTemplateDTO.getAssignedModules())
			{
				ModuleKeyDTO moduleKeyDTO = assignedModuleDTO.getModule().getKey();
				List<PolicyKeyDTO> policies = assignedModuleDTO.getModule().getAssignedPolicies().stream().map(p -> p.getPolicy().getKey()).collect(Collectors.toList());
				consentDTO.getModuleStates().put(moduleKeyDTO, new ModuleStateDTO(moduleKeyDTO, assignedModuleDTO.getDefaultConsentStatus(), policies));
			}
			EM_RWL.readLock().unlock();
			unlocked = true;
	...
	public Set<SignerId> getSignerIdsForVP(long vpID)
	{
		VirtualPerson virtualPerson = em.find(VirtualPerson.class, vpID);
		if (virtualPerson == null)
			return Set.of();
		return virtualPerson.getVirtualPersonSignerIds().stream().map(VirtualPersonSignerId::getSignerId).collect(Collectors.toSet());
	}

	public DomainDTO updateDomainInUse(String domainName, Date lastCheckDate) throws UnknownDomainException
	{
		EM_RWL.writeLock().lock();
		try
		{
			Domain domain = getDomain(domainName);
			domain.setLastCheckTimestamp(new Timestamp(lastCheckDate.getTime()));
			return domain.toDTO();
		}
		finally
		{
	...
	public Long getVirtualPersonForConsentKey(ConsentKeyDTO consentKeyDTO)
			throws UnknownSignerIdTypeException, UnknownConsentTemplateException, InvalidVersionException, UnknownDomainException, UnknownConsentException
	{
		EM_RWL.readLock().lock();
		try
		{
			// NPE will not occur because getConsentObject() throws UnknownConsentException
			return getConsentObject(consentKeyDTO).getKey().getVirtualPersonId();
		}
		finally
		{
			EM_RWL.readLock().unlock();
