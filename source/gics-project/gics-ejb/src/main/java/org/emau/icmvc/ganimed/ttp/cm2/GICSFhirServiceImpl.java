...
public class GICSFhirServiceImpl extends GICSServiceBase implements GICSFhirService
{
	...
	// QC types
	private static final String DEFAULT_VALID_QC_TYPES = "not_checked,checked_no_faults,checked_minor_faults";
	private static final String DEFAULT_INVALID_QC_TYPES = "checked_major_faults,invalidated";
	private static final String DEFAULT_QC_TYPE = "not_checked";
	...
	public String exportDefinition(String domainName, FhirExportMode exportMode, List<String> itemList, boolean exportLogo, String fileFormat) throws InternalException, InvalidExchangeFormatException
	{
		...
		return convertToExchangeFormat(def, fileFormat, false);
	}

	ExchangeFormatDefinition prepareExport()
	{
	 ...
	 */
	String convertToExchangeFormat(ExchangeFormatDefinition def, String fileFormat, boolean pretty) throws InvalidExchangeFormatException
	{
		IParser parser;
		registerFhirTypes();
		...
		return parser.encodeResourceToString(def);
	}
	 ...
	 * 		domain is not known
	 */
	private void exportDefinition(DomainDTO domain, ExchangeFormatDefinition def,
			Boolean exportLogo) throws UnknownDomainException
	{
		logger.debug("Exporting ALL");
		exportDefinition(domain, def, exportLogo,
				dad.listPolicies(domain.getName(), false),
				dad.listModules(domain.getName(), false),
				dad.listConsentTemplates(domain.getName(), false));
	}

	void exportDefinition(DomainDTO domain, ExchangeFormatDefinition def, Boolean exportLogo,
			List<PolicyDTO> policies, List<ModuleDTO> modules, List<ConsentTemplateDTO> templates)
	{
	...
	private List<Type> convertToFhirConsentTemplates(List<ConsentTemplateDTO> listConsentTemplates)
	{
		...
		for (ConsentTemplateDTO gicsTempl : listConsentTemplates)
		{
			...
			if (gicsTempl.getValidFromProperties() != null)
			{
				fhirTempl.setValidFromProperties(new ValidFromPropertiesObject(gicsTempl.getValidFromProperties()).toPropertiesString());
			}
	...
	{
		return switch (gicsType)
		{
			case STRING -> ConsentTemplateFreeText.FreeTextType.STRING;
			case DATE -> ConsentTemplateFreeText.FreeTextType.DATE;
			case BOOLEAN -> ConsentTemplateFreeText.FreeTextType.BOOLEAN;
			case INTEGER -> ConsentTemplateFreeText.FreeTextType.INTEGER;
			case DOUBLE -> ConsentTemplateFreeText.FreeTextType.DOUBLE;
		};
	}
	...
	private PatientConsentStatus convertToFhirConsentStatus(ConsentStatus status)
	{
		...
		{
			case ACCEPTED -> PatientConsentStatus.ACCEPTED;
			case DECLINED -> PatientConsentStatus.DECLINED;
			case UNKNOWN -> PatientConsentStatus.UNKNOWN;
			case NOT_ASKED -> PatientConsentStatus.NOT_ASKED;
			case NOT_CHOSEN -> PatientConsentStatus.NOT_CHOSEN;
			case WITHDRAWN -> PatientConsentStatus.WITHDRAWN;
			case INVALIDATED -> PatientConsentStatus.INVALIDATED;
			case REFUSED -> PatientConsentStatus.REFUSED;
			case EXPIRED -> PatientConsentStatus.EXPIRED;
			case OBJECTED -> PatientConsentStatus.OBJECTED;
			case ACCEPTED_OPT_OUT -> PatientConsentStatus.ACCEPTED_OPT_OUT;
		};
	}
	...
	ConsentDomain convertToFhirConsentDomain(DomainDTO existingDomain, Boolean exportLogo)
	{
		...
		convert.setConfig(new ConsentDomainConfig(existingDomain.getConfig()));
		convert.setCreationDate(existingDomain.getCreationDate());
			...
			if (domainName.equals(importedTemplate.getDomainName()))
			{
				...
				try
				{
					...
					importTemplateDTO = new ConsentTemplateDTO(
							...
							new ValidFromPropertiesObject(importedTemplate.getValidFromProperties()).toDTO(), importedTemplate.getComment(),
				...
				catch (UnknownConsentTemplateException maybe)
				{
					try
					{
						...
						addedTemplates.add(importTemplateKeyDTO);
						   ...
						   RequirementsNotFullfilledException | UnknownDomainException | UnknownConsentTemplateException | IllegalCompositionException | VersionConverterClassException e)
					{
						ignoredTemplates.add(importTemplateKeyDTO);
				...
				catch (UnknownDomainException maybe)
				{
					...
					else
					{
					   ...
					   ObjectInUseException | DateTimeParseException | ParseException | InvalidParameterException | RequirementsNotFullfilledException | IllegalCompositionException e)
				{
					ignoredTemplates.add(importTemplateKeyDTO);
					String errorMessage = "Error while processing template " + importedTemplate.toKeyString();
					logger.error(errorMessage, e);
			...
			if (domainName.equals(currentModule.getDomainName()))
			{
				...
				catch (UnknownModuleException maybe)
				{
					...
					if (!preview)
					{
						...
						catch (UnknownDomainException | DuplicateEntryException | IllegalCompositionException | InvalidVersionException | RequirementsNotFullfilledException | UnknownPolicyException |
							   VersionConverterClassException e)
						{
							ignoredModules.add(moduleKeyDTO);
							String errorMessage = "Error while processing module " + currentModule.toKeyString();
							logger.error(errorMessage, e);
				...
				catch (InvalidParameterException | InvalidVersionException | IllegalCompositionException | ObjectInUseException | RequirementsNotFullfilledException
	...
	private Set<AssignedPolicyDTO> createAssignedPolicies(ConsentModule currentModule, boolean preview, List<Type> assignedPolicies) throws InvalidExchangeFormatException
	{
		...
		for (Type typedPolicy : assignedPolicies)
		{
			...
			try
			{
				...
				{
					gicsAssignedPolicyDTO.setExpirationProperties(new ExpressionExpirationPropertiesObject(policy.getExpirationProperties()).toDTO());
				}
			}
			...
			if (domainName.equals(p.getDomainName()))
			{
				...
				catch (UnknownPolicyException maybe)
				{
					...
					if (!preview)
					{
						...
						catch (UnknownDomainException | DuplicateEntryException | InvalidVersionException | RequirementsNotFullfilledException | VersionConverterClassException e)
						{
	 ...
	 * 		true if the processing should be simulated and not persited
	 */
	private void processImportedDomainInfo(ConsentDomain newDomain, boolean allowUpdates, boolean preview) throws InvalidExchangeFormatException
	{
		DomainDTO newDomainDTO = convertFromFhirConsentDomain(newDomain);
					...
					if (existingDomain.getFinalised())
					{
						...
						if (!preview)
						{
							dad.updateDomainInUse(existingDomain.getName(), newDomainDTO.getLabel(), newDomainDTO.getLogo(),
									newDomainDTO.getExternProperties(), null, newDomainDTO.getComment(), newDomainDTO.getConfig());
		...
		catch (ObjectInUseException | ParseException | VersionConverterClassException e)
		{
			ignoredDomains.add(existingDomain);
			logger.error(e.getLocalizedMessage());
		}
	...
	DomainDTO convertFromFhirConsentDomain(ConsentDomain newDomain) throws InvalidExchangeFormatException
	{
		String logo = null;
		...
		DomainConfig config = convertFromFhirConsentDomainConfig(newDomain.getName(), newDomain.getConfig(), newDomain.getProperties());
		ExpirationPropertiesDTO expirationProperties = null;

		try
		{
			expirationProperties = new ExpirationPropertiesObject(newDomain.getExpirationProperties()).toDTO();
		}
		catch (ParseException e)
		{
			// intentionally ignored here but will be checked later in #processImportedDomainInfo(...)
		}

		return new DomainDTO(newDomain.getName(), newDomain.getLabel(),
				newDomain.getConsentTemplateVersionConverter(), newDomain.getModuleVersionConverter(),
				newDomain.getPolicyVersionConverter(), config, newDomain.getComment(),
	...
	DomainConfig convertFromFhirConsentDomainConfig(String domainName, ConsentDomainConfig newConfig, String newProperties) throws InvalidExchangeFormatException
	{
		DomainConfig config;
		if (newConfig == null && !StringUtils.isBlank(newProperties))
		{
			// fix properties-string problems
			config = new DomainConfig(domainName, removeBracketsAndSpaces(newProperties));
		}
		else
		{
			config = newConfig != null ? newConfig.toDomainConfig() : new DomainConfig();
		}

		QualityControlConfig qc = config.getQualityControlConfig();
		// check if import domain contains QC properties
		if (qc.getDefaultTypeId() == null || qc.getDefaultTypeId().equals(QCDTO.AUTO_GENERATED))
		{
			// add default QC types
			qc.setDefaultTypeId(DEFAULT_QC_TYPE);
			qc.getValidQcTypeValues().addAll(Arrays.stream(DEFAULT_VALID_QC_TYPES.split(",")).map(String::trim).toList());
			qc.getInvalidQcTypeValues().addAll(Arrays.stream(DEFAULT_INVALID_QC_TYPES.split(",")).map(String::trim).toList());
			qc.getValidQcTypeValues().remove(QCDTO.AUTO_GENERATED);
			qc.getInvalidQcTypeValues().remove(QCDTO.AUTO_GENERATED);
			qc.normalize(domainName);
		}
		return config;
	}
	 ...
	 * @throws UnknownDomainException
	 */
	public void finaliseDomain(String domainName) throws UnknownDomainException, VersionConverterClassException
	{
	...
	ExchangeFormatDefinition validateFormat(String definition, String fileFormat) throws InvalidExchangeFormatException
	{
		try
		{
			registerFhirTypes();
			ExchangeFormatDefinition importDef;
				...
				if (checkImportVersion(importDef.getSupportedVersion()))
				{
					...
					try
					{
						for (Type t : importDef.getTemplates())
						{
							...
							// ensure that a template always has a non-empty name
	 ...
	 *
	 * @param version
	 * 		the version string to test for compatibility with the current version
	 */
	 ...
	 * NOTE: this function is obsolete after successfull registration of custom fhir types under
	...
	private void registerFhirTypes()
	{
		...
		getFhirContext().registerCustomType(org.emau.icmvc.magic.fhir.datatypes.config.ConsentQCType.class);
		getFhirContext().registerCustomType(org.emau.icmvc.magic.fhir.datatypes.config.ConsentQCProblemTypeAction.class);
		getFhirContext().registerCustomType(org.emau.icmvc.magic.fhir.datatypes.config.ConsentQCProblemType.class);
		getFhirContext().registerCustomType(org.emau.icmvc.magic.fhir.datatypes.config.ConsentNotificationsConfig.class);
		getFhirContext().registerCustomType(org.emau.icmvc.magic.fhir.datatypes.config.ConsentQualityControlConfig.class);
		getFhirContext().registerCustomType(org.emau.icmvc.magic.fhir.datatypes.config.ConsentPoliciesConfig.class);
		getFhirContext().registerCustomType(org.emau.icmvc.magic.fhir.datatypes.config.ConsentScansConfig.class);
		getFhirContext().registerCustomType(org.emau.icmvc.magic.fhir.datatypes.config.ConsentStatisticConfig.class);
		getFhirContext().registerCustomType(org.emau.icmvc.magic.fhir.datatypes.config.ConsentApplicationConfig.class);
		getFhirContext().registerCustomType(org.emau.icmvc.magic.fhir.datatypes.config.ConsentDomainConfig.class);
