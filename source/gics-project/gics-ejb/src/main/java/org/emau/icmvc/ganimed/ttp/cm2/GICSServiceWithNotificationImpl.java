...
public class GICSServiceWithNotificationImpl extends GICSServiceBase implements GICSServiceWithNotification
{
	...
	{
		logger.info("add {} with notificationClientID {}", consentDTO, notificationClientID);
		checkParameter(consentDTO, "consentDTO");
		checkAllowedDomain(consentDTO);
		dad.addConsent(consentDTO, notificationClientID);
		logger.info("added {} with notificationClientID {}", consentDTO, notificationClientID);
	}

	@Override
	public void addConsentOptOut(String notificationClientID, ConsentTemplateKeyDTO templateKeyDTO, Set<SignerIdDTO> signerIdDTOs)
			...
			InvalidFreeTextException, UnknownDomainException, MissingRequiredObjectException, MandatoryFieldsException, DuplicateEntryException
	{
		...
		checkParameter(templateKeyDTO, "templateKeyDTO");
		checkAllowedDomain(templateKeyDTO);
		ConsentDTO consentDTO = dad.addConsentOptOut(templateKeyDTO, signerIdDTOs, notificationClientID);
			...
			UnknownSignerIdTypeException, UnknownConsentException, InvalidParameterException
	{
		...
		{
			logger.info("update{} with notificationClientID {}", createLogMessageForUpdateConsent(keyDTO, externProperties, comment, scan), notificationClientID);
		}
		...
		if (logger.isInfoEnabled())
		{
			logger.info("updated{} with notificationClientID {}", createLogMessageForUpdateConsent(keyDTO, externProperties, comment, scan), notificationClientID);
		}
	}
			...
			UnknownDomainException, InternalException, InvalidParameterException
	{
		if (logger.isInfoEnabled())
		{
			logger.info("refuse{} with notificationClientID {}", createLogMessageForRefuseConsent(ctKeyDTO, signerIdDTOs), notificationClientID);
		}
		...
		if (logger.isInfoEnabled())
		{
			logger.info("refused {} with notificationClientID {}", createLogMessageForRefuseConsent(ctKeyDTO, signerIdDTOs), notificationClientID);
		}
	}
	...
	{
		logger.debug("setting quality control {} for {} with notificationClientID {}", qc, consentKeyDTO, notificationClientID);
		...
		logger.debug("done setting quality control {} for {} with notificationClientID {}", qc, consentKeyDTO, notificationClientID);
	}
}
