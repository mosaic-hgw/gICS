...
public class ConsentCache
{
			...
			Map<ConsentKey, Long> legalConsentDates)
	{
		...
		try
		{
			...
			List<CachedSignedPolicy> vpSignedPolicies = domainVPCache.computeIfAbsent(vpId, k -> new ArrayList<>());
			...
			{
				SignedPolicyKey signedPolicyKey = entry.getKey().getKey();
				ConsentKey consentKey = signedPolicyKey.getConsentKey();
				if (entry.getValue() != null)
				{
					SignedPolicyKey newSignedPolicyKey = signedPolicyKey;
					//if virtual persons are different (addSignerId propably called), create same key again but with the new virtualPerson, otherwise the consentKey will be the same for different virtualPersons
					//but this also replicates ConsentKeys which only differ in vpIDs increasing memory of cache
					if (!consentKey.getVirtualPersonId().equals(vpId))
						newSignedPolicyKey = new SignedPolicyKey(new ConsentKey(consentKey.getCtKey(), consentKey.getConsentDate(), vpId), signedPolicyKey.getPolicyKey());
					SignedPolicyKey cachedKey = getCachedSignedPolicyKey(newSignedPolicyKey);

					// QC-GELB: if  policyCache.getPolicy(cachedKey.policyKey).validQcTypes.contains(qc.getStatusType)

					CachedSignedPolicy cachedSignedPolicy = new CachedSignedPolicy(cachedKey, gicsConsentDates.get(consentKey), legalConsentDates.get(consentKey),
	...
	public static void removeConsent(Consent consent, Set<SignerIdKey> signerIdKeys, Map<SignedPolicy, Long> signedPoliciesWithExpirationDates, String domainName)
	{
		...
		try
		{
			...
			for (Entry<SignedPolicy, Long> entry : signedPoliciesWithExpirationDates.entrySet())
			{
				if (entry.getValue() != null)
				{
					CachedSignedPolicy cachedSignedPolicy = new CachedSignedPolicy(entry.getKey().getKey(), consent.getConsentDateValues().getGicsConsentTimestamp(),
	...
	public static List<CalcUtils.ChangedSignedPolicyValidity> getChangedPolicies(String domain, Long virtualPersonId, Date fromDate, Date toDate)
	{
		CACHE_RWL.readLock().lock();
		try
		{
			if (VP_CACHE.get(domain) != null && VP_CACHE.get(domain).get(virtualPersonId) != null)
			{
				return CalcUtils.getChangedPolicyValidities(VP_CACHE.get(domain).get(virtualPersonId), fromDate, toDate);
			}
		}
		finally
		{
	...
	public static List<CalcUtils.ChangedSignedPolicyValidity> getChangedPolicies(String domain, Date fromDate, Date toDate)
	{
		CACHE_RWL.readLock().lock();
		try
		{
			if (VP_CACHE.get(domain) != null)
			{
				return CalcUtils.getChangedPolicyValidities(VP_CACHE.get(domain).values().stream().flatMap(Collection::stream).collect(Collectors.toList()), fromDate, toDate);
			}
		}
		finally
		{
	...
	public static final class CachedSignedPolicy
	{
