...
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.DELETION_DOMAIN;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.DELETION_TEST;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.DOMAIN;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.DOMAIN_CONFIG;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.EXPIRATION_PROPERTIES;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.VERSION_DOMAIN;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.allConsentStatus;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.am11;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.am1121;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.am1122;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.am2231;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.amForDeletion;
...
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.ctKey11;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.ctKey12;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.ctKey13;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.ctKey21;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.ctKeyForDeletion;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.domainNameForUpdateTest;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.idTypes;
...
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.moduleForDeletion;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.moduleKey11;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.moduleKey1121;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.moduleKey1122;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.moduleKey1131;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.moduleKey2231;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.moduleKeyForDeletion;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.nonExistingPolicyKey;
...
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.policyForDeletion;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.policyKey11;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.policyKey21;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.policyKey22;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.policyKey31;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.policyKeyForDeletion;
import static org.emau.icmvc.ganimed.ttp.cm2.TestHelper.signer_id;

@Tag("Integration")

public class CM2ManagerTests extends AbstractServiceBasedTest
{
	/*-
		loeschskript:
	...
	{
		System.setProperty("org.apache.cxf.Logger", Logger.class.getName());
		logger.info("setup");
		...
		try
		{
			logger.warn("reusing cached domain: " + cm2Service.getDomain(DOMAIN) + " !!!");
		}
		catch (UnknownDomainException maybe)
		{
			try
			{
				...
				cm2Manager.addSignerIdType(DOMAIN, "additional_dummy_sid");
				DomainDTO dummyDomain = cm2Service.getDomain(DOMAIN);
				Assertions.assertEquals(dummyDomain.getSignerIdTypes().size(), 3);
			}
		...
		Assertions.assertEquals(EXPIRATION_PROPERTIES, cm2Service.getDomain(DOMAIN).getExpirationProperties());

		try
		{
			cm2Service.getDomain(VERSION_DOMAIN);
		...
		catch (UnknownDomainException maybe)
		{
			try
			{
				DomainDTO domainDTO = new DomainDTO(VERSION_DOMAIN, "dummy", MajorMinorVersionConverter.class.getName(),
						MajorMinorVersionConverter.class.getName(), MajorMinorMaintenanceVersionConverter.class.getName(),
						DOMAIN_CONFIG, "version-test-domain",
						"no extern properties", "logo", idTypes, true, null, null, EXPIRATION_PROPERTIES, null, null);
		...
		Assertions.assertEquals(EXPIRATION_PROPERTIES, cm2Service.getDomain(VERSION_DOMAIN).getExpirationProperties());
	...
	public void basicTest() throws Exception
	{
		...
		catch (IllegalCompositionException expected)
		{
			logger.info("expected IllegalCompositionException: " + expected);
		}
		...
		tempCT = cm2Service.getConsentTemplate(ctKey11);
		...
		PolicyDTO tempPolicy = cm2Service.getPolicy(policyKey11);
		...
		ModuleDTO tempModule = cm2Service.getModule(moduleKey11);
			...
			UnknownSignerIdTypeException, UnknownConsentException, InvalidParameterException
	{
		...
		List<ConsentTemplateDTO> all = cm2Service.listConsentTemplates(DOMAIN, true);
		// je nachdem, welche tests schon ausgefuehrt wurden und mit jedem test werden es 2 mehr, darum ohne obergrenze
		Assertions.assertTrue(all.size() > 2, "wrong number of consent templates: " + all.size());
		for (ConsentTemplateDTO dto : all)
		{
			logger.info(dto);
		}
		logger.info("get the newest version of consent template 'testCT1'");
		ConsentTemplateDTO newest = cm2Service.getCurrentConsentTemplate("testCT1", DOMAIN);
		...
		List<ConsentTemplateDTO> currentCTs = cm2Service.listCurrentConsentTemplates(DOMAIN);
		for (ConsentTemplateDTO ct : currentCTs)
		{
			logger.info(ct);
		}
		logger.info("list all consents without scans");
		List<ConsentLightDTO> consents = cm2Service.getAllConsentsForDomainWithoutScan(DOMAIN);
		for (ConsentLightDTO consentLight : consents)
		{
			ConsentDTO consent = cm2Service.getConsent(consentLight.getKey());
		...
		long countWithoutFilter = cm2Service.countConsentsForDomainWithFilter(DOMAIN, pConfig);
		logger.info("count without filter: " + countWithoutFilter);
		pConfig.setFilter(filterMap);
		long countWithFilter = cm2Service.countConsentsForDomainWithFilter(DOMAIN, pConfig);
		...
		long countWithOrFilter = cm2Service.countConsentsForDomainWithFilter(DOMAIN, pConfig);
		...
		do
		{
			...
			consents = cm2Service.getConsentsForDomainPaginated(DOMAIN, pConfig);
		...
		while (!consents.isEmpty());
		Assertions.assertEquals(validConsentCount, invalidOrConsentCount, "validConsentCount != invalidOrConsentCount");
		...
		List<QCHistoryDTO> qcHist = cm2Service.getQCHistoryForConsent(aConsentKeyDTO);
		Assertions.assertFalse(qcHist.isEmpty(), "no qc history found");
	...
	public void consentTest() throws Exception
	{
		...
		cm2Service.getConsentTemplate(ctKeyConsentTest).getAssignedModules().stream().forEach(am -> Assertions.assertEquals(EXPIRATION_PROPERTIES, am.getExpirationProperties()));
		...
		{
			cm2Service.addConsent(consent);
			Assertions.fail("could create a consent with an invalid module");
		}
		catch (UnknownModuleException expected)
		{
			logger.info("expected UnknownModuleException: " + expected);
		...
		Assertions.assertTrue(cm2Service.isConsented(ids, policyKeyForConsentTestMandatory, config),
				"isConsented() returned 'false' where it should return 'true'");
		Assertions.assertTrue(cm2Service.isConsentedFromIncludingToIncluding(ids, DOMAIN, policyNameMandatory, "1", "1", config),
				"isConsented(from, to) returned 'false' where it should return 'true'");
		Assertions.assertFalse(cm2Service.isConsentedFromExcludingToIncluding(ids, DOMAIN, policyNameMandatory, "1", "1", config),
				"isConsented(from, to) returned 'true' where it should return 'false'");
		Assertions.assertFalse(cm2Service.isConsentedFromIncludingToExcluding(ids, DOMAIN, policyNameMandatory, "1", "1", config),
				"isConsented(from, to) returned 'true' where it should return 'false'");
		Assertions.assertFalse(cm2Service.isConsentedFromExcludingToExcluding(ids, DOMAIN, policyNameMandatory, "1", "1", config),
				"isConsented(from, to) returned 'true' where it should return 'false'");
		...
		try
		{
			cm2Service.isConsented(ids, nonExistingPolicyKey, config);
		...
		Assertions.assertTrue(cm2Service.isConsented(ids, policyKeyForConsentTestMandatory, config),
				"isConsented() returned 'false' where it should return 'true'");
		Assertions.assertTrue(cm2Service.isConsentedFromIncludingToIncluding(ids, DOMAIN, policyNameMandatory, "x", "x", config),
				"isConsented(from, to) returned 'false' where it should return 'true'");
		try
		{
			cm2Service.isConsented(ids, nonExistingPolicyKey, config);
		...
		cm2Service.addScanToConsent(consentKey, text, txtType, fileName);
		consent = cm2Service.getConsent(consentKey);
		...
		cm2Service.updateConsentInUse(consentKey, newExternProperties, newComment, scan);
		consent = cm2Service.getConsent(consentKey);
		...
		List<ConsentLightDTO> consentsForPerson = cm2Service.getAllConsentsForSignerIds(DOMAIN, ids, false);
	...
	public void versionTest() throws Exception
	{
		...
		try
		{
			DomainDTO domainDTO = new DomainDTO("crash-test-dummy", "dummy", "abc.def.ghi", SimpleVersionConverter.class.getName(),
					SimpleVersionConverter.class.getName(), DOMAIN_CONFIG, "crash-test-domain", "no extern properties", "", idTypes, false, null, null, EXPIRATION_PROPERTIES, null, null);
		...
		try
		{
			DomainDTO domainDTO = new DomainDTO("crash-test-dummy", "dummy", SimpleVersionConverter.class.getName(),
					SimpleVersionConverter.class.getName(), "abc.def.ghi", DOMAIN_CONFIG, "crash-test-domain", "no extern properties", "", idTypes, false, null,
					null, EXPIRATION_PROPERTIES, null, null);
			logger.info("creating crash test domain: " + domainDTO);
			cm2Manager.addDomain(domainDTO);
		...
		try
		{
			PolicyKeyDTO versionPolicyKey = new PolicyKeyDTO(VERSION_DOMAIN, "test", "12345.6.7");
			PolicyDTO versionPolicy = new PolicyDTO(versionPolicyKey);
			cm2Manager.addPolicy(versionPolicy);
			Assertions.fail("could create a policy with an invalid version string");
		}
	...
	public void domainTests() throws Exception
	{
		...
		String newExternProperties = "externProperties_" + date;
		String newComment = "comment_" + date;
		ExpirationPropertiesDTO newExpirationProperties = new ExpirationPropertiesDTO(EXPIRATION_PROPERTIES);
		newExpirationProperties.setFixedExpirationDate(Dates.toDate(Dates.tomorrow()));

		cm2Manager.updateDomainInUse(DOMAIN, newLabel, newLogo, newExternProperties, newExpirationProperties, newComment, null);
		DomainDTO domain = cm2Service.getDomain(DOMAIN);
		...
		Assertions.assertEquals(newExpirationProperties, domain.getExpirationProperties());
		cm2Manager.updateDomainInUse(DOMAIN, newLabel, newLogo, newExternProperties, null, newComment, null);
		domain = cm2Service.getDomain(DOMAIN);
		Assertions.assertEquals(new ExpirationPropertiesDTO(null, Period.of(1, 2, 3)), domain.getExpirationProperties());
		...
		try
		{
			...
			DomainDTO createDomainDTO = new DomainDTO(domainNameForUpdateTest, "dummy", SimpleVersionConverter.class.getName(), SimpleVersionConverter.class.getName(),
					SimpleVersionConverter.class.getName(), DOMAIN_CONFIG, "test-domain", "no extern properties", "logo", idTypes, false, null, null, EXPIRATION_PROPERTIES, null, null);
			...
			domain = cm2Service.getDomain(domainNameForUpdateTest);
			Assertions.assertEquals(newLabel, domain.getLabel());
			Assertions.assertEquals(newLogo, domain.getLogo());
			Assertions.assertEquals(newExternProperties, domain.getExternProperties());
			Assertions.assertEquals(newComment, domain.getComment());
	...
	public void signerIdTests() throws Exception
	{
		...
		try
		{
			cm2Service.addAlias(DOMAIN, aliasId, signerId);
			Assertions.fail("could add an alias to an unknown id");
		}
		catch (UnknownSignerIdException expected)
		{
			cm2Service.addAlias(DOMAIN, signerId, aliasId);
			List<SignerIdDTO> aliases = cm2Service.getAliasesForSignerId(DOMAIN, signerId);
			Assertions.assertTrue(containsSignerId(aliases, aliasId), "aliases doesn't contain the added alias");
			cm2Service.deactivateAlias(DOMAIN, signerId, aliasId);
			aliases = cm2Service.getAliasesForSignerId(DOMAIN, signerId);
		...
		// ein consent anlegen, damit die signerId vorhanden ist
		cm2Service.addConsent(consent);
		List<ConsentLightDTO> origConsents = cm2Service.getAllConsentsForSignerIds(DOMAIN, origIdSet, false);
		List<ConsentLightDTO> aliasConsents = cm2Service.getAllConsentsForSignerIds(DOMAIN, aliasIdSet, false);
		Assertions.assertEquals(1, aliasConsents.size(), "wrong consents fetched for signerId " + aliasId);
		List<ConsentLightDTO> allConsents = cm2Service.getAllConsentsForSignerIds(DOMAIN, bothIdSet, false);
		Assertions.assertEquals(allConsents.size(), origConsents.size() + 1, "wrong consents fetched for signerIds " + signerId + " + " + aliasId);
		Assertions.assertTrue(listEquals(allConsents, cm2Service.getAllConsentsForSignerIds(DOMAIN, origIdSet, true)), "wrong consents fetched for signerId " + signerId + " with aliases");
		Assertions.assertTrue(listEquals(aliasConsents, cm2Service.getAllConsentsForSignerIds(DOMAIN, aliasIdSet, true)),
				"wrong consents fetched for signerId " + aliasId + " with non existing aliases");
		cm2Service.addAlias(DOMAIN, aliasId, signerId);
		Assertions.assertTrue(listEquals(allConsents, cm2Service.getAllConsentsForSignerIds(DOMAIN, aliasIdSet, true)), "wrong consents fetched for signerId " + aliasId + " with existing aliases");
		cm2Service.deactivateAlias(DOMAIN, signerId, aliasId);
		Assertions.assertTrue(listEquals(allConsents, cm2Service.getAllConsentsForSignerIds(DOMAIN, aliasIdSet, true)), "wrong consents fetched for signerId " + aliasId + " with existing aliases");
		cm2Service.deactivateAlias(DOMAIN, aliasId, signerId);
		Assertions.assertTrue(listEquals(aliasConsents, cm2Service.getAllConsentsForSignerIds(DOMAIN, aliasIdSet, true)),
		...
		cm2Service.addSignerIdToConsent(consentKey, signerId);
	...
	public void deletionTests() throws Exception
	{
		...
		DomainDTO domainDTO = new DomainDTO(DELETION_DOMAIN, DELETION_TEST, SimpleVersionConverter.class.getName(),
				SimpleVersionConverter.class.getName(), SimpleVersionConverter.class.getName(), DOMAIN_CONFIG, "test-domain", "no extern properties", "logo",
				idTypes, true, null, null, EXPIRATION_PROPERTIES, null, null);
		...
		try
		{
			cm2Service.getConsentTemplate(ctKeyForDeletion);
		...
		try
		{
			cm2Service.getModule(moduleKeyForDeletion);
		...
		try
		{
			cm2Service.getPolicy(policyKeyForDeletion);
		...
		try
		{
			cm2Service.getDomain(DELETION_DOMAIN);
	...
	public void testFhirID() throws Exception
	{
		...
		try
		{
			...
			DomainDTO domainByName = cm2Service.getDomain(domainName);
			Assertions.assertNotNull(domainByName.getFhirID(), "fhirID is null");
			Assertions.assertFalse(domainByName.getFhirID().isEmpty(), "fhirID is empty");
			DomainDTO domainByFhirID = cm2Service.getObjectByFhirID(DomainDTO.class, domainByName.getFhirID());
			...
			module = cm2Service.getModule(moduleKey);
			Assertions.assertEquals('-', module.getFhirID().charAt(7), "fhirID of module isn't modified (see comment in Module.modifyFhirID)");
		}
		finally
		{
	...
	@Test
	public void mappingConsentTemplateTests() throws Exception
	{
		assertMappings(ctKey11, Set.of(), Set.of(), Set.of());
		assertMappings(ctKey21, Set.of(), Set.of(), Set.of());

		ct11.setMappedConsentTemplates(Set.of(ctKey21));
		cm2Manager.updateConsentTemplateInUse(ct11);
		assertMappings(ctKey11, Set.of(ctKey21), Set.of(), Set.of());
		assertMappings(ctKey21, Set.of(ctKey11), Set.of(), Set.of());

		ConsentTemplateDTO ct21 = cm2Service.getConsentTemplate(ctKey21);
		ct21.setMappedConsentTemplates(Set.of());
		cm2Manager.updateConsentTemplateInUse(ct21);
		assertMappings(ctKey11, Set.of(), Set.of(), Set.of());
		assertMappings(ctKey21, Set.of(), Set.of(), Set.of());
	}

	private static void assertMappings(ConsentTemplateKeyDTO ctKey,
			Set<ConsentTemplateKeyDTO> consentKeys,
			Set<ConsentTemplateKeyDTO> refusalKeys,
			Set<ConsentTemplateKeyDTO> revocationKeys)
			throws Exception
	{
		ConsentTemplateDTO ct = cm2Service.getConsentTemplate(ctKey);
		Assertions.assertEquals(consentKeys, ct.getMappedConsentTemplates());
		Assertions.assertEquals(revocationKeys, ct.getMappedRevocationTemplates());
		Assertions.assertEquals(refusalKeys, ct.getMappedRefusalTemplates());

		HashSet<ConsentTemplateKeyDTO> allCtKeys = new HashSet<>(consentKeys);
		allCtKeys.addAll(refusalKeys);
		allCtKeys.addAll(revocationKeys);

		Assertions.assertEquals(allCtKeys, ct.getAllMappedTemplates());
	}
}
