...
{
	private static final long serialVersionUID = -5539716233109218955L;
	private ModuleKeyDTO key;
	private ConsentStatus consentState;
	private final List<PolicyKeyDTO> policyKeys = new ArrayList<>();
	...
	public ModuleStateDTO(ModuleKeyDTO key, ConsentStatus consentState, List<PolicyKeyDTO> policyKeys)
	{
		...
		setPolicyKeys(policyKeys);
	}

	public ModuleStateDTO(ModuleStateDTO dto)
	{
		this(dto.getKey(), dto.getConsentState(), dto.getPolicyKeys());
	}
	...
	{
		if (this.policyKeys != policyKeys)
		{
			this.policyKeys.clear();
			if (policyKeys != null)
			{
				for (PolicyKeyDTO keyDTO : policyKeys)
				{
					this.policyKeys.add(new PolicyKeyDTO(keyDTO));
				}
			}
		}
	}
