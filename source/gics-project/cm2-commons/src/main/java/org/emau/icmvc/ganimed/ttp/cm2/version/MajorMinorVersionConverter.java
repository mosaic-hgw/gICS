...
/**
 * format: "x.y"; 0 <= x <= 2146; 0 <= y <= 999<br>
 * also valid: "x"; 0 <= x <= 2146<br>
 * attention: a.0b = a.b
 * 
 * @author geidell
 * 
...
{
	private static final String MSG = "invalid version format '%s' - it should be x.y were 0 <= x <= 2146 and 0 <= y <= 999";

	@Override
	public String intToString(int version) throws InvalidVersionException
	{
		if (version < 0 || version > 2146999)
		{
			throw new InvalidVersionException("version must be between 0 and 2146999 - current value is " + version);
		}
