...
public enum ConsentStatus
{
	...
	OBJECTED(ConsentStatusType.DECLINED), ACCEPTED_OPT_OUT(ConsentStatusType.ACCEPTED);
	...
	public static String getStringForType(ConsentStatusType csType)
	{
		...
		if (!csList.isEmpty())
		{
			sb.append(csList.getFirst().name());
	...
	public static List<ConsentStatus> getOrderedStatusesForConsentTemplateType(ConsentTemplateType type, List<ConsentStatus> statuses)
	{
		List<ConsentStatus> ordered = new ArrayList<>();

		switch (type)
		{
			case CONSENT, CONSENT_OPT_OUT:
				ordered.addAll(statuses.stream().filter(cs -> ConsentStatusType.ACCEPTED.equals(cs.getConsentStatusType())).toList());
				ordered.addAll(statuses.stream().filter(cs -> ConsentStatusType.DECLINED.equals(cs.getConsentStatusType())).toList());
				ordered.addAll(statuses.stream().filter(cs -> ConsentStatusType.UNKNOWN.equals(cs.getConsentStatusType())).toList());
				break;
			case REVOCATION, REFUSAL, OBJECTION:
				ordered.addAll(statuses.stream().filter(cs -> ConsentStatusType.DECLINED.equals(cs.getConsentStatusType())).toList());
				ordered.addAll(statuses.stream().filter(cs -> ConsentStatusType.UNKNOWN.equals(cs.getConsentStatusType())).toList());
				ordered.addAll(statuses.stream().filter(cs -> ConsentStatusType.ACCEPTED.equals(cs.getConsentStatusType())).toList());
				break;
			default:
				LogManager.getLogger(ConsentStatus.class).warn("Cannot order consentStatus for templateType {}", type);
				ordered.addAll(statuses);
		}
		return ordered;
	}
}
