 ...
 */
@ViewScoped
@Named("policyStatusController")
public class PolicyStatusController extends AbstractGICSBean
{
	@Serial
	...
	public void init()
	{
		try
		{
			availablePolicies = getService().listPolicies(domainSelector.getSelectedDomainName(), true).stream()
	...
	public void onLoadAllSignerIds() throws UnknownDomainException, UnknownSignerIdTypeException, InvalidParameterException
	{
		...
		for (String signerId : getService().getAllIdsForSignerIdType(domainSelector.getSelectedDomainName(), signerIdType))
		{
	...
	public void onRequestPolicy()
	{
		...
		try
		{
			for (String id : inputs)
			{
				...
				if (detailedResult)
				{
					policyResults.put(id, getService().getConsentStatusType(signerIdSet, policy.getKey(), checkConsentConfig).toString());
				}
				else
				{
					policyResults.put(id, String.valueOf(getService().isConsented(signerIdSet, policy.getKey(), checkConsentConfig)));
				}
