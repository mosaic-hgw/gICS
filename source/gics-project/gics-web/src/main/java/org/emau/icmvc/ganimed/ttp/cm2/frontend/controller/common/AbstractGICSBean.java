...
{
	@Serial
	private static final long serialVersionUID = 1386040663154586464L;
	...
	public QualityControlConfig getQcConfig()
	{
		return getSelectedDomain().getConfig().getQualityControlConfig();
	}
	...
	{
		return Objects.requireNonNull(getQcConfig().getTypes().stream().filter(q -> q.getId().equals(type)).findAny().orElse(null))
				.getLabelOrId(getBundle().getLocale().getLanguage());
	}
	...
	{
		return getSelectedDomain().getConfig().getNotificationsConfig().isSendFromWeb();
	}

	public List<String> getDomainSignerIdTypes()
	{
		return getSelectedDomain().getSignerIdTypes();
	}

	public List<ConsentTemplateType> getDomainTemplateTypes()
	{
		return new ArrayList<>(getSelectedDomain().getConfig().getApplicationConfig().getTemplateTypes());
	}

	public boolean useDetailedTypes()
	{
		return getDomainTemplateTypes().contains(ConsentTemplateType.CONSENT) && getDomainTemplateTypes().contains(ConsentTemplateType.CONSENT_OPT_OUT);
	}
	
	public String detailedTypes()
	{
		return useDetailedTypes() ? "detailed." : "";
	}
	...
	protected ResourceBundle getBundleFor(String language)
	{
	...
	public GICSService getServiceWithAutomaticNotification()
	{
		return getServiceWithAutomaticNotification(isUsingNotifications());
	}
}
