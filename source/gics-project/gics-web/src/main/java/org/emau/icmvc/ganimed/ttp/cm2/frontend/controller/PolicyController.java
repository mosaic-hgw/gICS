...
public class PolicyController extends AbstractConsentController
{
	...
	public void onDuplicatePolicy(PolicyDTO policy)
	{
		...
		editPolicy.getKey().setName(null);
	...
	public void onSavePolicy(Boolean scroll)
	{
		...
		if (pageMode == PageMode.NEW)
		{
			try
			{
				...
				editPolicy.getKey().setName(editPolicy.getKey().getName().trim());
				editPolicy.setFinalised(true);
				getManager().addPolicy(editPolicy);
			...
			catch (UnknownDomainException | RequirementsNotFullfilledException | InvalidParameterException | VersionConverterClassException e)
			{
				FacesContext.getCurrentInstance().validationFailed();
		...
		else if (pageMode == PageMode.EDIT)
		{
			...
			{
				editPolicy.setFinalised(true);
				getManager().updatePolicyInUse(editPolicy.getKey(), editPolicy.getLabel(), editPolicy.getExternProperties(), editPolicy.getComment());
	...
	public void onFinalise(PolicyDTO policy)
	{
		try
		{
			getManager().finalisePolicy(policy.getKey());
	...
	public void onDeletePolicy(PolicyDTO policy) throws UnknownDomainException
	{
		...
		try
		{
			getManager().deletePolicy(policy.getKey());
	...
	private void loadPolicies()
	{
		try
		{
			policies = getService().listPolicies(domainSelector.getSelectedDomainName(), false);
		}
