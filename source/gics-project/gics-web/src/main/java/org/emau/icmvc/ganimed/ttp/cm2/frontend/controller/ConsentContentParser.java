...
{
	@Serial
	private static final long serialVersionUID = -5049539372759504884L;

	public static final String TEMPLATE_MARKER = "template";

	private static final String OUTPUT_FOLDER = "";

	private static final float TEMPLATE_QR_CODE_SIZE = 120; // px, defined in consents.css
	private static final float MINIMAL_QR_CODE_SIZE_INCL_OFFSET = 111;
	...
	// wenig von
	private static final double DEFAULT_CIRCLE_EXTRACT_MARGIN = 0.20;
	...
	private Boolean checkForInvalidScaling(float y1, float y2)
	{
		...
		logger.debug("--- template qr code height={}, defined height={}px, percentage={}", qrCodeHeight, TEMPLATE_QR_CODE_SIZE, qrCodeHeight * 100 / TEMPLATE_QR_CODE_SIZE);
		if (qrCodeHeight < MINIMAL_QR_CODE_SIZE_INCL_OFFSET)
		{
	...
	private void extractAndProcessModuleStates(HashMap<Result, BufferedImage> moduleQrCodes)
	{
		...
		if (detectedTemplateDTO != null)
		{
			...
			List<AssignedModuleDTO> missingModules = modulesFromTemplate.stream().filter(m -> !m.getDisplayCheckboxes().isEmpty()).collect(Collectors.toList());
			...
			for (Entry<Result, BufferedImage> moduleQrCode : moduleQrCodes.entrySet())
			{
				...
				Result result = moduleQrCode.getKey();
				if (result.getText() != null && !result.getText().isEmpty() && image != null)
				{
					extractAndProcessModuleStates(result, image, detectedTemplateDTO, modulesFromTemplate, missingModules, detectedModules);
				}
	...
	private void extractAndProcessModuleStates(Result result, BufferedImage image, ConsentTemplateDTO detectedTemplateDTO, Set<AssignedModuleDTO> modulesFromTemplate, List<AssignedModuleDTO> missingModules,
			List<DetectedModuleDTO> detectedModules)
	{
		// process
		...
		List<Boolean> checkedStateList = storeAndInterpretPartialImage(image, 0,
				(int) result.getResultPoints()[0].getY(),
				image.getWidth(),
				(int) result.getResultPoints()[1].getY());

		// count marked
		int marked = (int) checkedStateList.stream().filter(Boolean.TRUE::equals).count();
		ModuleKeyDTO detectedModuleKey = ModuleQRCodec.DEFAULT.fromQRCodeToModuleKey(result.getText(), detectedTemplateDTO);
		...
		for (AssignedModuleDTO templateModule : modulesFromTemplate)
		{
			if (templateModule.getModule().getKey().equals(detectedModuleKey))
			{
				...
				List<ConsentStatus> states = templateModule.getDisplayCheckboxesOrderedForTemplateType(detectedTemplateDTO.getType());
	...
	private List<BufferedImage> extractPages(String filename)
	{
		...
		try
		{
			...
			File destinationFile = new File(OUTPUT_FOLDER);
			...
			{
				logger.error("{} File does not exist", sourceFile.getName());
			}
		}
		catch (Exception e)
		{
	...
	private List<Boolean> interpretMarkersFromPartialImage(Mat partialImage)
	{
		...
		if (detectedRatios.size() == 1)
		{
			...
			result.add(detectedRatios.getFirst() > MIN_THRESHOLD);
			return result;
		}
		...
		if (!src.empty())
		{
			...
			for (int x = 0; x < circles.cols(); x++)
			{
				...
				try
				{
					...
					int margin = (int) (radius * DEFAULT_CIRCLE_EXTRACT_MARGIN);
