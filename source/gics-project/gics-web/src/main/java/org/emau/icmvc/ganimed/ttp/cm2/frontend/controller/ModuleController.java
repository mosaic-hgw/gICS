...
public class ModuleController extends AbstractConsentController
{
	@Serial
	...
	private Map<PolicyKeyDTO, TimeProperty> policyExpirations = new HashMap<>();
	...
	public void onEditModule(ModuleDTO module) throws UnknownDomainException, InvalidVersionException, InvalidParameterException
	{
		...
		{
			logMessage(getBundle().getString("module.message.info.isFinal"), Severity.INFO, false);
		}
	}

	public ModuleDTO onDuplicateModule(ModuleDTO module, boolean copyName) throws UnknownDomainException, InvalidVersionException, InvalidParameterException
	{
		pageMode = PageMode.NEW;
		editModule = new ModuleDTO(module.getKey());
		if (!copyName)
		{
			editModule.getKey().setName(null);
		}
	...
	public void onSaveModule(Boolean scroll)
	{
		...
		if (pageMode == PageMode.NEW)
		{
			try
			{
				...
				editModule.getKey().setName(editModule.getKey().getName().trim());
				getManager().addModule(editModule, false);
			...
			catch (IllegalCompositionException e)
			{
				...
				logMessage(new MessageFormat(getBundle().getString("module.message.error.composition")).format(exceptionArgs), Severity.WARN, scroll);
			}
			...
			catch (UnknownDomainException | UnknownPolicyException | RequirementsNotFullfilledException | InvalidParameterException | VersionConverterClassException e)
			{
				FacesContext.getCurrentInstance().validationFailed();
		...
		else if (pageMode == PageMode.EDIT)
		{
			try
			{
				...
				{
					getManager().updateModuleInUse(editModule.getKey(), editModule.getLabel(), editModule.getShortText(), editModule.getExternProperties(), editModule.getComment(),
							editModule.getAssignedPolicies());
				}
				else
				{
					getManager().updateModule(editModule, false);
				}
	...
	public void onFinalise(ModuleDTO module)
	{
		try
		{
			getManager().finaliseModule(module.getKey(), true);
	...
	public void onDeleteModule(ModuleDTO module) throws UnknownDomainException, UnknownModuleException
	{
		...
		try
		{
			getManager().deleteModule(module.getKey());
	...
	public void addNewestPolicy() throws UnknownDomainException, InvalidVersionException, InvalidParameterException
	{
		...
		{
			List<PolicyDTO> newPolicies = getService().listPolicies(domainSelector.getSelectedDomainName(), false);
	...
	public void refreshAvailablePolicies() throws UnknownDomainException, InvalidVersionException, InvalidParameterException
	{
		...
		List<PolicyDTO> unassignedPolicies = getService().listPolicies(domainSelector.getSelectedDomainName(), false);

		unassignedPoliciesLoop:
		for (PolicyDTO policy : unassignedPolicies)
		{
			for (AssignedPolicyDTO assignedPolicyDTO : policies.getTarget())
			{
				...
				{
					continue unassignedPoliciesLoop;
				}
			}
	...
	private void loadModules()
	{
		try
		{
			modules = getService().listModules(domainSelector.getSelectedDomainName(), false);
		}
	...
	private void loadPolicies(ModuleDTO module) throws InvalidVersionException, UnknownDomainException, InvalidParameterException
	{
		...
		// Get all available policies
		List<PolicyDTO> allPolicies = getService().listPolicies(domainSelector.getSelectedDomainName(), false);
		...
		for (AssignedPolicyDTO assignedPolicy : module.getAssignedPolicies())
		{
			...
			expiration.getEditTimeProperty().setExpirationProperties(assignedPolicy.getExpirationProperties());
		...
		for (PolicyDTO policy : allPolicies)
		{
			...
			policyExpirations.put(policy.getKey(), new TimeProperty());
		}
	...
	public void onAssignPolicies()
	{
		...
		for (AssignedPolicyDTO policy : policies.getTarget())
		{
			...
			if (!found)
			{
				...
				policyExpirations.put(policy.getPolicy().getKey(), new TimeProperty());
			}
		}
	...
	private void preparePolicies()
	{
		for (AssignedPolicyDTO policy : editModule.getAssignedPolicies())
		{
			...
			TimeProperty expiration = policyExpirations.getOrDefault(policy.getPolicy().getKey(), new TimeProperty());
			ExpirationPropertiesDTO expirationProperties = expiration.getSavedTimeProperty().getExpirationProperties();
			String expirationExpression = expirationProperties instanceof ExpressionExpirationPropertiesDTO expressionExpirationPropertiesDTO ? expressionExpirationPropertiesDTO.getExpirationExpression() : null;
			policy.setExpirationProperties(new ExpressionExpirationPropertiesDTO(expirationProperties.getFixedExpirationDate(), expirationProperties.getValidPeriod(), expirationExpression));
		}
	}
	...
	public Map<PolicyKeyDTO, TimeProperty> getPolicyExpirations()
	{
